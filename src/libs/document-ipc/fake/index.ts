...
export type DocumentIpcFake = Readonly<{
   ...
   *
   * 全コマンドを失敗させる代役だと監視も張れず、「ファイルが不正な状態」に到達できないため、
   * 書き込みだけを落とせるようにしている。
   */
   ...
   *
   * 代役の書き込みは即座に解決するため、そのままでは「書き込み中」の状態を画面から観測できな
   * い。押した直後の見え方（ボタンが押せないこと）を確かめるために止められるようにしている。
   *
   * @param path 止める対象のパス
