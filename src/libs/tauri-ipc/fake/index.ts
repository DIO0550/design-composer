...
/**
 * イベントを 1 種類だけ配る `TauriIpc` の代役。コマンドは持たない（呼ばれたら本物と同じく
 * 「そのコマンドは無い」で reject する）。
 *
 * 購読だけを包む境界（`AppMenu` / `FileDrop`）の代役はどれもこの形になるので、イベント名と
 * 拒否の仕方をここ 1 つに集める。差し込む先が `TauriIpc` の位置なので、これを渡した境界では
 * 本体の解釈（イベント名の照合・payload の検証）がそのまま動く。
 */
