...
test("major がアプリより新しいファイルを読むとアプリの更新を促すエラーになる", () => {
  const text = `{ "formatVersion": "3.0", "tokens": {}, "components": {}, "artboards": [] }`;
...
test("minor がアプリより新しいファイルを読むとアプリの更新を促すエラーになる", () => {
  const text = `{ "formatVersion": "2.7", "tokens": {}, "components": {}, "artboards": [] }`;
...
test("major がアプリより古いファイルは変換できないとエラーになる（major 0 からのステップが無いため）", () => {
...
test("書き出しは常に現在の形式を名乗る", () => {
  ...
  expect(text).toContain(`"formatVersion": "2.0"`);
});
...
test("違う minor を名乗るドキュメントを書き出しても読み直せる", () => {
  ...
  expect(reloaded.formatVersion).toEqual({ major: 2, minor: 0 });
});
