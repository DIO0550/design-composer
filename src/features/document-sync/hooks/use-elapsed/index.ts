...
import { Elapsed } from "@/domains/unit/elapsed";
 ...
 *
 * 購読を張るのは起点があるあいだだけ（常に張ると、数えるものが無いときまで毎秒の再レン
 * ダーが走る）。他の再描画のついでに数え直す形にしないのは、数えている相手が操作を受け
 * 付けない画面で再描画の機会がほとんど無く、`0s` のまま止まって見えるため。
 *
