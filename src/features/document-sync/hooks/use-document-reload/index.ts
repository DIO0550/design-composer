 ...
 * 監視は開いている間ずっと張り、直近の IPC の失敗を返して呼び出し側が表示を決める。
 *
 * 見えていないドキュメントでも張り続ける。解除すると Rust 側が把握している内容を忘れ、
 * 張り直すときに現在の内容を「把握済み」として記録するので、解除していた間の外部変更が
 * 届かないまま消える（`src-tauri/src/document/watch.rs`）。
 *
