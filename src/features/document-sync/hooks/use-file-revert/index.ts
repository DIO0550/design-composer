...
/**
 * 外部変更で不正になったファイルを、表示中の内容で上書きして捨てる（UI 案 docs/Design Composer.html の Error 画面
 * `revert file`）。書き込みはボタンの押下から始めるので `useEffect` には置かない。
 *
 * `useAutoSave` に任せられないのは、あちらの契機が「表示中のドキュメントが最後に書き出したものと違うこと」で、書き戻しは
 * 表示中のドキュメントを変えないため一度も条件に当たらないから。
 *
 * 成功を `onReverted` で伝えるのは、アプリ自身の書き込みが外部変更として返ってこない（Rust 側が自書き込みを識別して通知を
 * 止める / #27）ため。待っていてもエラー一覧が畳まれない。
 *
