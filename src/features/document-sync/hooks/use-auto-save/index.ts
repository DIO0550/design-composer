 ...
 * ため 1 つにまとめる。
 *
 * 「書き出してよい内容」を `Option<DesignDocument>` にして渡さないのは、`Option.some`
 * が呼ぶたびに新しいオブジェクトを作り、依存配列に入れると再レンダーのたびに effect が
 * 張り直されてデバウンスが取り直されるため。
 */
 ...
 *
 * 契機を個々の編集アクションではなく `document` の値の変化に置いているのは、undo の適用
 * も「通常の編集」として同じ経路で書き出すため（#29）。保存は last-write-wins で、書き
 * 込み前に読み直したりマージしたりはしない。
 *
