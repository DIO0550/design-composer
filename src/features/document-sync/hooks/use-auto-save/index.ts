...
/**
 * 書き出す内容と書き出し先、および書き出してよいか。どれか 1 つでは書き込みが決まらないため 1 つにまとめる。
 *
 * 「書き出してよい内容」を `Option<DesignDocument>` にして渡さないのは、`Option.some` が呼ぶたびに新しいオブジェクトを
 * 作り、依存配列に入れると再レンダーのたびに effect が張り直されてデバウンスが取り直されるため。
 */
...
/**
 * ドキュメントの変更をデバウンスしてファイルへ書き出す（docs/05-architecture.md「保存モデル: 自動保存」）。直近の
 * 書き込み失敗を返し、呼び出し側が表示を決める。
 *
 * 契機を個々の編集アクションではなく `document` の値の変化に置いているのは、undo の適用も「通常の編集」として同じ経路で
 * 書き出すため（#29）。保存は last-write-wins で、書き込み前に読み直したりマージしたりはしない。
 *
 * ファイルが不正な間は書き出さない。映っているのは最後に正常だった表示なので、書き出すと**より新しい外部の書き込みを
 * 古い内容で潰す**ことになる（表示中の内容で潰す操作は `revert file` が別に持つ）。
 *
