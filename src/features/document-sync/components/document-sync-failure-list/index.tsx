 ...
 * 描かない。
 *
 * 自動保存・外部変更の監視・書き戻しをまとめて 1 つの一覧にするのは、どれが失敗しても利
 * 用者に伝わる意味が同じ（画面の内容とファイルの中身がずれているかもしれない）だから。
 * 並びは同期が起きる順（自動で書く → 外から届く → 明示的に書き戻す）に固定する。
 *
 * **この一覧が出すのは経路のラベルと診断用の原文だけで、`reason` は読まない。** 開く経
 * 路（`ioFailureLabel`）のように理由ごとの日本語を出さないのは、同期の失敗が「何をして
 * いて失敗したか」で伝わるため。
 *
 * `DocumentAccessFailure[]` を受け取らず経路ごとに受けるのは、失敗の値そのものがどれで
 * 起きたかを持たないため。
 */
