 ...
 * 存モデル: 自動保存」「外部編集の検知」/ #136 の書き戻し）。失敗が 1 つも無ければ何も
 * 描かない。
 *
 * 自動保存・外部変更の監視・書き戻しをまとめて 1 つの一覧にするのは、どれが失敗しても伝
 * わる意味が同じ（画面の内容とファイルの中身がずれているかもしれない）だから。並びは同
 * 期が起きる順（自動で書く → 外から届く → 明示的に書き戻す）に固定する。
 *
 * **この一覧が出すのは経路のラベルと診断用の原文だけで、`reason` は読まない。** 理由ご
 * との日本語を出さないのは同期の失敗が「何をしていて失敗したか」で伝わるためで、経路ご
 * とに受けるのは失敗の値そのものがどれで起きたかを持たないため。
 */
