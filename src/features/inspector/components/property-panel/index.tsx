 ...
 *
 * サブフォルダに `__tests__/` は置かない。テストは `PropertyPanel` を通した
...
/**
 * 右ペインの帯に出す、いま選んでいるもの（UI 案 docs/Design Composer.html のインスペクタの 44px の帯）。
 *
 * 帯そのもの（`PaneHeading`）は呼び出し側が置く（どのペインへ着せるかは 3 ペインの組み立ての判断で、この feature は持たない）。
 * 選んでいないときに中身だけを空にするのはそのためで、帯ごと消すと選択のたびに本文の位置が帯のぶん動く。
 *
 * 複数選んでいるときに件数を出すのは、1 つの名前も種別も決まらないため（docs/06-ui.md「選択」。本文は編集欄を出さないので、ここが
 * 唯一の手がかりになる）。
 *
...
/**
 * プロパティパネル（docs/06-ui.md「画面構成」。UI 案 docs/Design Composer.html のインスペクタ）。右ペインの帯に出す見出しと、その下の
 * 本文の 2 つに分かれる。
 *
 * 1 つの部品にまとめて器（`EditorLayout.RightPane`）ごと返さないのは、器が編集画面の組み立て（`features/editor`）に属していて、この
 * feature からは import できないため。呼び出し側が帯と本文それぞれの器に入れる。
 *
 * 帯と本文は**呼び出し側が同じ 1 つの `selection` を両方へ渡す前提**で、同じ純粋関数（`SelectionControls.forSelection`）を通す。別々の
 * 選択を渡せば「帯は件数なのに本文はインスタンスの編集欄」が作れるので、器を着せる側（`rightPaneParts`）で 1 つの値を作って配る。
 *
 * 入力欄はスキーマ定数の走査だけで決まるため、ここには prop 名で分岐するコードを置かない。
 */
