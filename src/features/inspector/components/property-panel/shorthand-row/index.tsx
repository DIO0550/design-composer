...
/**
 * 4 辺を 1 行にまとめた行（UI 案 docs/Design Composer.html の `padding`）。半幅セルを 2 列のグリッドに詰めるところまで UI 案と同じ。
 *
 * 既定を畳んだ 2 欄にするのは Figma と同じ形にするため（#230）。UI 案は 4 辺の状態しか描いていないので、切り替えボタンの見た目はここで
 * 決めている。
 *
 * 切り替えを `useState` で持つのは、畳んでいるかがドキュメントではなく画面の状態だから（docs/03「畳み方は表示の都合なので持たない」）。
 * 道具の状態なので**同じ行が出続ける間（Box 系を選び直す間）は残る**（この行を持たない Text を選ぶと行ごと消えるので、戻ったときは
 * 畳んだ状態から始まる）。
 *
