 ...
 * （`FieldBinding.value` は未設定・不揃いを空文字で表す）と受け取る向き（`<select>` /
 * `<input>` は空欄で空文字を返す）の両方に効く。
 *
 * ドメインへ渡す前にここで解釈するのは、文字列 prop にとって `""` はそれ自体が正当な値に
 * なりうるため（`PropControl` 側で決めると、その値にとっての意味が固定される）。
 *
 ...
 *
 * 読み上げへ繋ぐのは、この数値が `<select>` の読み上げ（トークン名だけ）からは得られない
 * 情報だから（色の見本が `aria-hidden` なのは、何の色かを隣のトークン名が既に伝えているた
 * めで事情が違う）。数値を欄の内側に置かないのは、`<select>` の中に要素を描けないため。
 *
