 ...
 *
 * `features/editor/features/sidebar` の `artboard-list` が `+` に与える名前と
 * **同じ綴りにしてある**（同じ操作に別の名前を与えない）。
 */
 ...
 * 色の青になる。`□` / `T` が既に同じ乖離を抱えており（別の単位で扱う）、ここだけ寄せる
 ...
 *
 * ボタンにはしない。`Assets` が browse-only で挿入がドラッグ専用である以上、これを押して挿
 * す経路は UI 案に存在しない。押せない見た目のまま置くと「挿せる位置が無いから押せない」と
 * 読めてしまうので、操作ではなく状態として描く。
 *
...
/**
 * キャンバスに浮かぶツールバー（UI 案 docs/Design Composer.html）。artboard を 1 枚足す
 * `#` と、プリミティブを選択位置へ挿す `□` / `T` が並ぶ（`◆` は** ボタンではなく状態表示**）。
 *
