...
import {
  EditableText,
  TextEdit,
} from "@/features/editor/features/canvas/domains/text-edit";
...
export type TextEditControl = Readonly<{
   ...
   * `names` は押された位置から外へ辿ったノード名。始まるのは**選択中の Text を押した
   * とき**だけで、それ以外（まだ内側があるとき・掘りきった先が Text でないとき・
   * 別の枝を押したとき）は何も起きない。
   */
...
/**
 * 掘りきった Text へのダブルクリックを「文言のその場編集」として解釈する（docs/06-ui.md
 * 「キャンバス直接操作」）。ダブルクリックそのものは 1 階層内側へ掘る操作で、掘る先が無
 * くなったときだけこちらの意味になる。
 *
 * このフックが持つのは DOM の実測とイベントの仲介だけで、「どれを編集できるか」「下書き
 * がどんな編集になるか」の判定は `text-edit` にある。
 *
...
export function useTextEdit(
  ...
  const start = (names: readonly string[]) => {
    ...
    const bounds = DrawnBounds.measure(text.value.name);
    if (!Option.isSome(bounds)) {
      return;
    }
    dispatch({ type: "start", text: text.value, bounds: bounds.value });
  };
