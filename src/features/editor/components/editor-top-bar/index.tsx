 ...
 *
 * `Document*` ではなく `Editor*` なのは、並ぶものがドキュメントの話に閉じないため（倍率はキャンバスの見え方で、ドキュメントには
 * 保存しない）。`<header>`（banner）にしているのは、画面の上端の帯がこれ 1 本だけだから（#374）。
 *
 * UI 案は左端に macOS の信号機ボタンを描いているが置いていない。`src-tauri/tauri.conf.json` が `decorations` を指定しておらず既定
 * （OS がウィンドウ装飾を描く）なので、装飾が二重になり、押しても閉じない偽の閉じるボタンが並ぶため。
 *
 * 高さ（`h-[38px]`）を落としても中身の分だけ縮むだけで**テストは 1 件も落ちない**。気づく手段は Storybook の視覚差分だけ。
 */
 ...
 *
 * 保存状態と入れ替えるのは、ファイルが不正な間に「保存済み」と名乗ると、映っているものがファイルに載っていると読めてしまうため
 * （映っているのは最後に正常だった表示で、ファイルの現在の中身とは違う / #135）。
 *
 * 「凍結中なら必ず 1 件以上」を型で縛る（`Option<非空の一覧>` を受ける）形を採らないのは、同じ判断が `DocumentReload` で既に済んで
 * おり、そこでの理由（起こらない空配列の分岐を書く羽目になる）がここでも変わらないため。
 *
...
/**
 * 倍率の操作（UI 案の `− 55% +`）。右端へ寄るのはこの並び自身の性質なので `ml-auto` を持つ（外すとパンくずの隣へ寄るが、
 * **テストは 1 件も落ちない**。気づく手段は Storybook の視覚差分だけ）。
 *
 * 倍率の表示そのものを等倍へ戻すボタンにしている。`等倍に戻す` を 4 つ目として並べないのは、UI 案の倍率の並びが `−` / 倍率 / `+` の
 * 3 つしか描いておらず、描かれていない操作は既存の流儀へ寄せるため（rules/ui-verification.md）。
 *
...
/**
 * 今映っているのが最後に正常だった表示で、それがどれだけ古いか（UI 案 docs/Design Composer.html の Error 画面の
 * `showing last valid render · 4s ago`）。
 *
 * 単位の綴り（`s` / `m` / `h`）と `ago` をここが持つのは、ドメイン（`Elapsed`）が持つのが単位と数までだから
 * （rules/architecture.md「表示のための綴りをドメインへ持ち込まない」）。
 *
 * `ml-auto` を持たないのは、右端へ寄せる役目を隣の `CanvasZoom` が既に持っているため。2 つ置くと余白が 2 つの auto マージンへ等分され、
 * `CanvasZoom` が帯の中ほどへ落ちる（**テストでは落ちない** — happy-dom は Tailwind を解決しない。気づく手段は視覚差分だけ）。
 *
