...
const meta = {
  ...
  // 殻が無いと中身の高さまで伸び、下端のドックが撮影のビューポートの外へ出る。
  decorators: [
    (Story) => (
      <ScreenHeightShell>
        <Story />
...
/**
 * ファイルとの同期に失敗している状態。実体の無いパスは監視を張れないので、失敗の帯が編集画
 * 面の上に出る。表示そのものは保たれることをここで確認できる。
 */
...
/**
 * ドキュメント自身が不正な状態。アプリ内の編集で使用中トークンを消したあとも、その内容が自
...
/**
 * 外部編集でファイルが壊れた編集画面。**このストーリーだけが 3 ペインの凍結を一度に映す**
