...
/*
 * artboard の追加・削除・並べ替えを編集画面の配線ごと確かめる。
 * 追加の入口は `Artboards` の一覧とキャンバスのツールバーの `#`、
 * 並べ替えは一覧、削除はキーボードの割り当てとコンテキストメニューの行なので、ここを
 * 通さないと「押すと実際にキャンバスが変わる」を守るテストが無くなる（メニューの行からの
 * 経路は `opened-document-editor.context-menu`）。
 *
 ...
 * 凍結中にツールバーごと出ないからなので、消えることをここで留める。
