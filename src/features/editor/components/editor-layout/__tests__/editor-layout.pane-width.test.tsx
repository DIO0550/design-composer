...
/**
 * ストーリーの枠（`RightPaneShell`）は右ペインの幅を写しているので、編集画面のグリッドだけ
 * を直すと絵が黙って古くなる。両方を描いて突き合わせられるのはこの層しかないため、写してい
 * る側ではなく写される側のテストとしてここに置く。
 */
...
/**
 * レール（`LeftPaneRail` の `w-14` = 56px）の幅（rem）。左ペインのパネル部は、器の左列
 * からこの分だけ狭い。
 *
 * 弱点は、レール幅を変えたときにこのテストが「パネル部（15.5rem）を動かせ」と誤って迫る
 * こと（本来動かすべきは列の側）。許容するのはレールが UI 案どおり 56px で安定しており、
 * レール自身のストーリーの視覚差分が別途その変更を捉えるためで、**レール幅を変えたらこ
 * の値も直す。**
 */
