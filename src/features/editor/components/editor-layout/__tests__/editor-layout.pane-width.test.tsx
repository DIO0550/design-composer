...
/**
 * レール（`LeftPaneRail` の `w-14` = 56px）の幅（rem）。左ペインのパネル部は、器の左列からこの分だけ狭い。
 *
 * ここに写しで持つのは、本物のレールを描画して読めないため（`LeftPaneRail` は `features/sidebar` の公開 API に出ておらず、この
 * editor 側のテストからレール本体へ deep import できない）。
 *
 * 弱点は、レール幅を変えたときにこのテストが「パネル部（15.5rem）を動かせ」と誤って迫ること（本来動かすべきは列の側）。許容するのは
 * レールが UI 案どおり 56px で安定しており、レール自身のストーリーの視覚差分が別途その変更を捉えるためで、
 * **レール幅を変えたらこの値も直す。**
 */
