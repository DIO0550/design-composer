...
/**
 * エラーがどこの不正を指しているか（#128）と、そこで何ができるか（#136）。由来ごとに画
 * 面での意味が違うので、一覧を出す側が取り違えないよう名前で指せるようにする（rules/coding.md
 * 「値の集合から union を導出する」）。
 *
...
/**
 * 由来ごとの見え方。読み上げ名・見出しの相手・置かれ方は常に対で決まるので、1 つの
 * switch で返す（3 つに分けると由来を足すときの直し方が 3 通りになる）。
 *
 * ファイル由来は下端へ密着するので上辺だけを仕切り、ドキュメント由来は下端に積む器の中
 * で浮くので枠を閉じて影を付ける。ドキュメント由来にも `absolute` を持たせてツールバー
 * の高さぶん上へずらす案を採らないのは、寸法をここへ写すことになり、ずれても happy-dom
 * は Tailwind を解決しないのでテストでは気づけないため（#128）。
 *
 ...
 *
 * 「表示中のドキュメントに在るか」までを条件にしないのは、行ごとに可否を配ると「行 + 飛
 * べるか」の対の型が要り、`errors` を渡すだけで描けるこの部品がエディタの状態に依存する
