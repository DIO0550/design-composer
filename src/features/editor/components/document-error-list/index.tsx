 ...
 * 一覧を出す側が取り違えないよう名前で指せるようにする（rules/coding.md「値の集合から
 * union を導出する」）。
 *
 ...
 *
 * ファイル由来は下端へ密着するので上辺だけを仕切り、ドキュメント由来は下端に積む器の中で浮
 * くので枠を閉じて影を付ける。ドキュメント由来にも `absolute` を持たせて上へずらす案を採ら
 * ないのは、寸法を写すことになり、happy-dom では気づけないため。
 *
 * @param origin エラーの由来
...
/**
 * エラーの並び。行の識別は並びの位置ではなく中身で行う（一覧は読み込みのたびに丸ごと入れ
 * 替わる）。
 *
 * 場所の綴りは識別にも表示にも要るので、ここで 1 度だけ作って渡す。
 *
 * @returns エラー 1 件ごとの行
 */
