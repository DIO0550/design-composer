 ...
 * き換えた経路ぶんになる。
 *
 * 今表示しているドキュメント（`present`）を履歴の外に置かず中に持つのは、undo が「今の
 * ドキュメント」と「積んである並び」が揃って初めて結果が決まるため。これにより履歴を積
 * まずにドキュメントだけ差し替える経路が無くなる。
 *
 * どちらの並びも時系列（古い → 新しい）で持つ。`past` の末尾が 1 つ前、`future` の先頭
 * が 1 つ先になる。
 */
