...
/**
 * 編集の履歴（docs/06-ui.md「編集操作の一覧」の undo / redo、docs/05-architecture.md「保存モデル」のメモリ内管理）。
 *
 * ドキュメントは不変なので、差分ではなくスナップショットを並べるだけで戻せる（#41）。書き換えた経路以外の枝は前のスナップショットと
 * 同じ参照が残るため、1 編集で増えるのは書き換えた経路ぶんになる。
 *
 * 今表示しているドキュメント（`present`）を履歴の外に置かず中に持つのは、undo が「今のドキュメント」と「積んである並び」が揃って初めて
 * 結果が決まるため。これにより履歴を積まずにドキュメントだけ差し替える経路が無くなる。
 *
 * どちらの並びも時系列（古い → 新しい）で持つ。`past` の末尾が 1 つ前、`future` の先頭が 1 つ先になる。
 */
