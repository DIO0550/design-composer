 ...
 * 幅を固定と書きながら長さの無い `broken`、部品インスタンスの `action` が並ぶ状態。
 */
...
function handlesOf(name: string): readonly string[] {
  return NodeResize.handles(EditorState.select(setupState(), name)).map(
    (handle) => handle.axis,
  );
}

test("何も選んでいないときはハンドルが出ない", () => {
  expect(NodeResize.handles(setupState())).toEqual([]);
});
...
test("ハンドルは掴んだ時点の長さを持つ", () => {
  expect(NodeResize.handles(EditorState.select(setupState(), "panel"))).toEqual(
    [
...
test("artboard のハンドルは artboard 自身の大きさを持つ", () => {
  expect(NodeResize.handles(EditorState.select(setupState(), "home"))).toEqual([
