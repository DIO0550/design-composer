 ...
 * 追加直後のトークンに入れる値。
 *
 * 色を黒にしているのは、白だと一覧の色見本が背景と同化して「足したのに見えない」ことになるため。長さは 0 で、足した時点で見た目を変えない値が
 * 直す前の既定として素直なため。
 *
 * 影と書体だけは 0 や空から始めない（影を 0/0/0 にすると一覧の見本にもキャンバスにも何も出ない）。どちらも docs/04-tokens.md「初期トークンセット」が
 * 挙げている値（`shadows.sm` / `typography.body`）を選んだ。デフォルトテーマとは値が一致するだけで参照はしていない（追加直後の見え方はこちらの関心事）。
 */
const InitialValues = {
