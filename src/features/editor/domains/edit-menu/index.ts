...
/**
 * 右クリックの対象（docs/06-ui.md「コンテキストメニュー」の 3 つ）。
 *
 * 押された場所が決めるもので、選択からは導けない。空き領域では選択に手を付けないので、
 * ノードを選んだまま空き領域のメニューが出る。
 */
...
export const EditMenuTarget = {
  /**
   * 押された位置から外へ辿った名前から対象を決める。
   *
   * 名前は内側から外側へ並び、**末尾は必ずその artboard 自身**（キャンバスが枠の名前を
   * 末尾に足す）。したがって 1 つだけなら artboard の枠か見出しを押したことになる。
   *
 ...
 *
 * キーは対象を引く見出しで、値は並びなので据え置く
 * （rules/naming.md「対応表のキーを PascalCase にするのは「キーが値の別名」のときだけ」）。
 *
 * docs が挙げる並びのうち、**実装が無い操作**（複製・名前を変更・グループ化・解除）と、
 * **名前を打つ欄を伴う部品化**は並べない。状態に依らず永久に押せない行は入口として
 * 働かないため（docs が「押せない状態で並べる」と定めているのは、状態によってできない
 * ものだけ）。
 */
 ...
 * 今その操作ができるか。
 *
 * 状態側が「その操作は存在しない」と答えるか（`none` を返すか）で決め、押せるかどうかの
 * 規則をここで書き直さない。答えを得るために状態を組み立てて捨てるが、走るのはメニューが
 * 開いている間だけ。
 *
