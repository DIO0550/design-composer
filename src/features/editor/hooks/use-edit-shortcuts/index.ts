 ...
 *
 * 削除（#39。artboard の削除もこの導線 / #43）・コピー & ペースト（#40）・undo / redo（#41）・選択解除（#414）・並べ替え（#417）・座標の移動（#413）は
 * キーボードだけの操作。UI 案（docs/Design Composer.html）が対応するボタンを持たないため、画面にも置いていない（#112 / #316）。
 *
 * 対象が無いときは状態側が「その操作は存在しない」と答える（各アクションの `none`）ので、押せるかどうかをここで判定しない。
 */
