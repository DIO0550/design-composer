 ...
 *
 * コピー & ペースト（#40）と undo / redo（#41）はキーボードだけの操作。
 * UI 案（docs/Design Composer.html）の左ペインが `Artboards` の `+` しか持たず、
 * 編集操作をボタン列で並べていないため、対応するボタンを置いていない。
 * 削除だけはボタンからも届くが、どちらも選択中のものを消すので同じアクションで受ける。
 *
