 ...
 *
 * 削除（#39。artboard の削除もこの導線 / #43）・コピー & ペースト（#40）・undo / redo（#41）
 * ・選択解除（#414）・並べ替え（#417）・座標の移動（#413）はキーボードだけの操作。UI 案
 * （docs/Design Composer.html）が対応するボタンを持たないため、画面にも置いていない（削
 * 除のボタンは #112 で外した。キャンバスに浮かぶツールバーが持つのは追加の入口と運んで
 * いることの表示だけで、削除は持たない / #316）。
 *
