...
/**
 * 選んでいるノードを兄弟の並びの中で 1 つぶん動かす操作を、キーボードから行えるようにする（docs/06-ui.md「編集操作の一覧」の並べ替え / #417）。
 *
 * このフックが持つのは「どのキーがどちらの向きか」だけで、ページ全体で受けることと入力中は無視することは `useKeyShortcuts` に任せる。
 *
 * 2 件をまとめて張るのは、押したキーで渡す向きが変わるため（1 つの割り当ての `keys` に 2 つ並べると同じ操作の別名になって区別できない）。表から割り当てを
 * 組み立てる形を `use-reposition-shortcut` と共有しないのは、畳むと渡す値の型を型引数で受けることになるため。
 *
