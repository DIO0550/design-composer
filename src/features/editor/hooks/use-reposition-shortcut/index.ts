...
/**
 * 選んでいる絶対配置のノードを少しずつ動かす操作を、キーボードから行えるようにする（docs/06-ui.md「キャンバス直接操作」/ #413）。
 *
 * このフックが持つのは「どのキーがどれだけの移動量か」だけで、ページ全体で受けることと入力中は無視することは `useKeyShortcuts` に任せる。
 *
 * 8 件をまとめて張るのは、押したキーで渡す移動量が変わるため（矢印を 1 つの `keys` に並べると同じ操作の別名になって向きを区別できない）。キーの綴りと
 * 刻みの大きさはこの入力経路の事情なので、渡すのは解釈済みの移動量にする（rules/architecture.md「入力欄の約束事をドメインへ持ち込まない」）。
 *
