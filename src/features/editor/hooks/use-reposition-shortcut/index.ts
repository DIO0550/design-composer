 ...
 *
 * 値そのものは docs/06-ui.md「キャンバス直接操作」にあるが、**10px を選んだ根拠**は docs
 * に無いのでここに置く。
 */
const MoveLengths = [
...
/**
 * 選んでいる絶対配置のノードを少しずつ動かす操作を、キーボードから行えるようにする（docs/06-ui.md
 * 「キャンバス直接操作」/ #413）。
 *
 * このフックが持つのは「どのキーがどれだけの移動量か」だけで、ページ全体で受けることと
 * 入力中は無視することは `useKeyShortcuts` に任せる。
 *
 * キーの綴りと刻みの大きさはこの入力経路の事情なので、渡すのは解釈済みの移動量にする（rules/architecture.md
 * 「入力欄の約束事をドメインへ持ち込まない」）。
 *
