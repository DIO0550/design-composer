 ...
 *
 * キーボードだけの操作（削除・コピー & ペースト・undo / redo・選択解除や並べ替え・座標の移動の割り当て）はここに含めない。画面の部品から呼ぶ相手が
 * おらず、`useEditShortcuts` が張るためだけに戻り値へ並べることになるため（削除は #112 でボタンを失ってこちら側になった）。
 *
 * `clearSelection` / `reorder` がここに在るのは、ツリーとインスペクタから呼ぶ相手が別に居るため。キーボードからの入口は対象の決め方が違う（選択から
 * 決める）ので、同じ操作でも別のアクションになる。
 *
