 ...
 * ル上段の `Artboards`）。ここで選んだ 1 枚の中身をツリーが映す。
 *
 * 見出しの右の `+` は UI 案そのもので、押すと末尾に 1 枚増えてそのまま選択になる。
 *
 * 並べ替えは行を掴んで運ぶ。docs/06-ui.md がドラッグと定めているのは**ノードの同一親内
 * の並べ替え**で、artboard の入口は定めておらず UI 案も描いていない。新しい形を発明せず、
 * 同じ左ペインのツリーと同じ機構（`useReorderDrag`）に載せている。
 *
 * 並びが 1 つしか無いので、落ちる先が並びの外を指すことは構造上ありえない（ツリー側は親
 * をまたげてしまうため、階層ごとに状態を分けて同じ性質を作っている）。
 *
