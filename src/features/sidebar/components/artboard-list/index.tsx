...
/**
 * artboard 1 枚の行（UI 案 docs/Design Composer.html の `# login 720×900`）。押すとその artboard が選択になりツリーも
 * 中身に入れ替わり、掴んで別の行の上で離すとその位置へ移る。
 *
 * `aria-current` が指すのは選択ではなく「今ツリーが映している 1 枚」。中のノードを選んでいる間も、それを載せている
 * artboard がここでは current になる。
 *
 * 幅・高さを読み上げから外すのは、行が指すのが artboard の名前だからで、こうしないと読み上げ名が大きさを含んだ文字列になる。
 */
...
/**
 * ドキュメントが持つ artboard の一覧（UI 案 docs/Design Composer.html の `Layers` パネル上段の `Artboards`）。ここで選んだ
 * 1 枚の中身をツリーが映す。見出しの右の `+` は UI 案そのもので、押すと末尾に 1 枚増えてそのまま選択になる。
 *
 * 並べ替えは行を掴んで運ぶ。docs/06-ui.md がドラッグと定めているのは**ノードの同一親内の並べ替え**で、artboard の入口は
 * 定めておらず UI 案も描いていない。新しい形を発明せず、同じ左ペインのツリーと同じ機構（`useReorderDrag`）に載せている。
 *
 * 並びが 1 つしか無いので、落ちる先が並びの外を指すことは構造上ありえない（ツリー側は親をまたげてしまうため、階層ごとに
 * 状態を分けて同じ性質を作っている）。
 *
 * artboard が 1 枚も無いことを伝えるのはここ。ツリー側は「今見ている 1 枚の中身」を映す場所だから。そのときも `+` は残す
 * （1 枚目を足す導線がここにしか無い）。
 */
