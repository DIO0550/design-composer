 ...
 *
 * 親の矩形はドキュメントに書かれていない（`hug` / `fill` があるので、大きさも位置もレイアウトを通すまで決まらない）ため実測でしか決められない。倍率は `transform` で
 * 効いており実測値に乗るので、ずれはドキュメント上の px へ割り戻す。落とし先が今の親と同じでも分岐しないのは、同じ要素を 2 回測ればずれが厳密に 0 になるため。
 *
 * 揃え先から**運んでいるノード自身を外す**のは、運んでいる間はその要素も一緒にずれて見えており、外さないと自分の辺との距離が常に最小になって**どこへ運んでも寄らなく
...
/**
 * 今の運び方。絶対配置のノードを運んでいるなら座標の置き直し、そうでなければツリーへの移動・挿入。
 *
 * 運び方を先に決めてから、その経路が要る親だけを解決する。落ちる先の親は 2 つの経路で条件が違い（座標は名前だけで決まり、並びへ挿す先は子が並ぶ向きまで要る）、1 回の
 * 解決を両方へ渡すと厳しいほうの条件が緩いほうまで塞ぐため（#440）。**走査は 1 回のまま** — どちらの運び方になるかは運んでいるものだけで決まる。
 *
 * **その運び方は実測の成否では変わらない。** 置き直しに決まったあとで実測に失敗したら、ツリーの移動へ落とさずそのまま「落とせない」にする（落とすと、座標を動かす
 * つもりのドラッグが黙って木の並びを書き換える別の編集になる）。
 *
 * @param context 今の掴みと、落とし先を決めるための材料
 * @returns 今の運び方
 */
...
/**
 * 掴んでキャンバスへ落とす操作を、ツリー上の位置への移動・挿入か、絶対配置のノードの座標の置き直し（親をまたげば付け替え）として解釈する
 * （docs/06-ui.md「キャンバス直接操作」/ docs/02-data-model.md「基本原則」）。
 *
 * このフックが持つのは DOM の実測とイベントの仲介だけで、「どこへ落ちるか」「いつドラッグとみなすか」の判定は `node-drop` / `node-drag` に、「実測した親のずれから
 * どの座標が書かれるか」は `reposition-target` に、「揃う辺があるならどれだけ寄せるか」は `side-snap` にある。
 *
 * ポインタキャプチャを使わないのは、捕捉すると以後のイベントの `target` が捕捉した要素に固定され、「今どのノードの上にいるか」を読めなくなるため。代わりに掴んだあとの
 * ポインタは 3 ペインの器全体で受ける（キャンバスの中だけで受けると、パレットの行で掴んで左ペインの上で離したときに `pointerup` が届かない）。
 *
