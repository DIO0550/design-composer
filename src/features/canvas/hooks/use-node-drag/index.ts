 ...
 *
 * 親の矩形はドキュメントに書かれていない（`hug` / `fill` があるので、大きさも位置もレイ
 * アウトを通すまで決まらない）ため実測でしか決められない。倍率は `transform` で効いてお
 * り実測値に乗るので、ずれはドキュメント上の px へ割り戻す。落とし先が今の親と同じでも
 * 分岐しないのは、同じ要素を 2 回測ればずれが厳密に 0 になるため。
 *
 * 揃え先から**運んでいるノード自身を外す**のは、運んでいる間はその要素も一緒にずれて見
...
/**
 * 掴んでキャンバスへ落とす操作を、ツリー上の位置への移動・挿入か、絶対配置のノードの座
 * 標の置き直し（親をまたげば付け替え）として解釈する（docs/06-ui.md「キャンバス直接操作」
 * / docs/02-data-model.md「基本原則」）。
 *
 * このフックが持つのは DOM の実測とイベントの仲介だけで、「どこへ落ちるか」「いつドラッ
 * グとみなすか」の判定は `node-drop` / `node-drag` に、「実測した親のずれからどの座標が
 * 書かれるか」は `reposition-target` に、「揃う辺があるならどれだけ寄せるか」は
 * `side-snap` にある。
 *
 * ポインタキャプチャを使わないのは、捕捉すると以後のイベントの `target` が捕捉した要素
 * に固定され、「今どのノードの上にいるか」を読めなくなるため。代わりに掴んだあとのポイ
