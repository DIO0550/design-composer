 ...
 * まま採って組み立てる。位置まで実測から採れないのは、運んでいる間は `transform` でずら
 * して見せており（`repositionPreviewDeclarations`）**前回の移動分が既に乗っている**ため。
 *
 * 大きさは `translate` で変わらないので実測を使う。
 *
 ...
 * 件が違い（座標は名前だけで決まり、並びへ挿す先は子が並ぶ向きまで要る）、1 回の解決を両
 * 方へ渡すと厳しいほうが緩いほうを塞ぐため（#440。運び方は運んでいるものだけで決まる）。
 *
 ...
 * 固定され、「今どのノードの上にいるか」を読めなくなるため。掴んだあとのポインタを 3 ペイ
 * ンの器全体で受けるのは、パレットの行で掴んで左ペインで離すと `pointerup` が届かないから。
 *
