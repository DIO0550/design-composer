...
/**
 * 掘りきった Text へのダブルクリックを「文言のその場編集」として解釈する（docs/06-ui.md「キャンバス直接操作」）。ダブルクリックそのものは 1 階層内側へ掘る操作で、
 * 掘る先が無くなったときだけこちらの意味になる。
 *
 * このフックが持つのは DOM の実測とイベントの仲介だけで、「どれを編集できるか」「下書きがどんな編集になるか」の判定は `text-edit` にある。
 *
 * 確定した文言を `PropEdit` として渡すのは、プロパティパネルからの `content` の編集と同じ経路（`apply_prop_edit`）に載せるため（自動保存もその経路に乗る）。
 *
