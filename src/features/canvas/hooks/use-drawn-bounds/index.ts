...
/**
 * 描かれている矩形を、変わるたびに追いかけて返す（リサイズハンドルを辺に重ねるために使う）。
 *
 * **依存配列を持たず毎コミット測り直す。** 位置が動く原因はキャンバス自身のドキュメント / 倍率だけでなく、トークン選択やエラー一覧の出入りで中央ペインの高さが変わる
 * ことでも起きるため、原因を列挙して deps に並べると上流の状態が増えるたびに追随が要る。測り直した値が前と同じなら**同じ参照を返して再レンダーを止める**。
 *
 * 拾えないのは**再レンダーを伴わない位置変化**（Web フォントや画像の読み込み完了）。ウィンドウの大きさの変化だけは再レンダーが起きないので明示的に購読する。
 *
