 ...
 * るかを確かめるのに使う。
 *
 * 本番の公開 API（`features/canvas/index.ts`）とは別の口にするのは、あちらへ出すと
 * `@testing-library/react` がアプリのバンドルへ入るため（`libs/<x>/fake/index.ts` が本
 * 番の入口と別に置かれているのと同じ形）。
 *
 * fixture を層直下（`src/features/__tests__/`）へ移して共有しないのは、ここの fixture
 * がキャンバスの内部（`TokenReferrerOutline`）を読んでおり、層直下からはその内部を引け
 * ないため。
 */
export {
