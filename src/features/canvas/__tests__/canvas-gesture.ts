...
/**
 * キャンバスへのポインタ / ホイール操作。キャンバス本体・ズーム / パンのフック・編集画面の通しから使うため、feature 直下に置いて共有する
 * （外の feature へはテスト用の公開口から出す）。
 *
 * ポインタそのものの操作は左ペインの並べ替えでも同じものが要るので、横断層（`components/__tests__/pointer-gesture`）へ移して**そのまま再輸出**する。
 * 包み直さないのは、`Offset` が `PointerPoint` と構造的に同じで型の上でも何も足せないため。ここが自前で持つのはホイール（キャンバス固有）だけ。
 */
