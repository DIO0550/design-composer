 ...
 * に対で意味を持つなら対を表す型を作る」）。
 *
 * `Edge` ではなく `Side` なのは、このリポジトリで 4 辺を指す語彙が `unit/side` の
 * `Side` で、`edge` は `CanvasBounds.edge` が**終端（右辺 / 下辺）だけ**を指す狭い意味
 * で既に使っているため（`Edge` で名付けると、grep した読み手が右下だけの話に着地する）。
 */
 ...
 *
 * `DropZone` と 1 つにまとめないのは、あちらが線の向きを `CssDirection`（子が並ぶ向き）
 * で、こちらが `SidePair`（揃った辺の組）で決めており、まとめると片方にもう片方の語彙が
 * 入るため。**揃えているのは値ではなく見せ方の約束**なので、拠り所は docs/06-ui.md 側に
 * 置いてある（片方だけ変えてもここは落ちない）。
 */
