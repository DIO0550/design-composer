...
/**
 * 辺のスナップ 1 回分（運んでいるものの行き先と、揃え先の並び。すべて画面上の px。docs/06-ui.md「キャンバス直接操作」）。
 *
 * 寄せ量は**運んでいるものの行き先と、揃え先の並びの両方**で決まり片方だけでは答えが出ないので、対を表す型にして判定をそこへ帰属させる
 * （rules/architecture.md「2つの値が常に対で意味を持つなら対を表す型を作る」）。
 *
 * `Edge` ではなく `Side` なのは、このリポジトリで 4 辺を指す語彙が `unit/side` の `Side` で、`edge` は `CanvasBounds.edge` が**終端（右辺 / 下辺）だけ**を指す狭い意味で
 * 既に使っているため（`Edge` で名付けると、grep した読み手が右下だけの話に着地する）。
 */
 ...
 * ガイド線の太さ（px）。
 *
 * 挿入位置に引く線（`DropZone` の `MarkerThicknessPx`）と同じ太さ・同じ中心合わせにして、キャンバスに出る線の流儀を割らない（2 つは排他なので、太さが同じでも
 * 混ざらない）。
 *
 * `DropZone` と 1 つにまとめないのは、あちらが線の向きを `CssDirection`（子が並ぶ向き）で、こちらが `SidePair`（揃った辺の組）で決めており、まとめると片方にもう
 * 片方の語彙が入るため。**揃えているのは値ではなく見せ方の約束**なので、拠り所は docs/06-ui.md 側に置いてある（片方だけ変えてもここは落ちない）。
 */
...
export const SideSnap = {
   ...
   * 揃うとみなす距離（**画面上の px**）。
   *
   * 倍率を変えても吸い付く手応えが変わらないようにするため画面上の px で持つ（ドキュメント上の px だと、拡大するほど広い範囲で吸い付いて狙った位置へ置けない）。
   */
  ThresholdPx: 6,
