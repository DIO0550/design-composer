...
export const EditableText = {
  /**
   * ダブルクリックされた位置にある、インライン編集できる Text。`names` は押された位置か
   * ら外へ辿ったノード名（キャンバスの選択と同じ読み方）。
   *
   * 対象を選択から決めるのは、確定した文言の書き込み先が選択中のものだから。ダブルクリ
   * ックは 1 階層ずつ内側へ掘る操作なので、選択が Text になるのは**掘りきったとき**で、
   * その次のダブルクリックがここで編集の開始になる（docs/06-ui.md「選択」）。押された位
   * 置も見るのは、選択中の Text から離れたところをダブルクリックしたときに編集が始まら
   * ないようにするため。
   *
