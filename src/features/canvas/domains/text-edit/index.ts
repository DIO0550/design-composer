...
export const EditableText = {
   ...
   *
   * 対象を選択から決めるのは、確定した文言の書き込み先が選択中のものだから。ダブルクリッ
   * クは 1 階層ずつ内側へ掘る操作なので、選択が Text になるのは**掘りきったとき**で、その
   * 次のダブルクリックがここで編集の開始になる（docs/06-ui.md「選択」）。
   *
   * 押された位置も見るのは、選択中の Text から離れたところをダブルクリックしたときに編集
   * が始まらないようにするため。
   *
