...
export const EditableText = {
  /**
   * ダブルクリックされた位置にある、インライン編集できる Text。`names` は押された位置から外へ辿ったノード名（キャンバスの選択と同じ読み方）。
   *
   * 対象を選択から決めるのは、確定した文言の書き込み先が選択中のものだから。ダブルクリックは 1 階層ずつ内側へ掘る操作なので、選択が Text になるのは**掘りきった
   * とき**で、その次のダブルクリックがここで編集の開始になる（docs/06-ui.md「選択」）。押された位置も見るのは、選択中の Text から離れたところをダブルクリックした
   * ときに編集が始まらないようにするため。
   *
   * artboard と部品インスタンスは対象にならない（文言を持つのは Text だけ）。インスタンスの中身に見えている Text も、ドキュメントの木には無いため対象にならない。
   *
   * @param selection 選択とドキュメントの出どころになる対
   * @param names 押された位置から外へ辿ったノード名
   * @returns 編集できる Text。選択が Text でないとき、押された位置が選択中の Text から
   *   外れているときは `none`
   */
  at(
    ...
    return Option.flatMap(forSelection(selection), (text) =>
...
export const TextEdit = {
   ...
   * `valueFrom` でそう読む）。パネルは既定値を別に見せる欄なので
