...
/**
 * 何かを掴んでからキャンバスで離すまでの状態（docs/06-ui.md「キャンバス直接操作」の移動と、「編集操作の一覧」の挿入）。
 *
 * 掴んだものを持つのは離すまで、落ちる先を持つのは動かしている間だけ、と状態ごとに持つものが変わるため直和で列挙する（「動かしていないのに落ちる先がある」を
 * 作れなくするため）。
 *
 * 掴むのはキャンバス上の既存ノードとパレットの雛形の 2 通りで、どちらを運んでいるかは `DraggedNode` が持つ。2 つのドラッグに分けないのは、分けるとキャンバスが
 * 落とし先を 2 つ受け取ることになり「両方ドラッグ中」が型で書けてしまうため。
 *
 * `dropped` は既存ノードを離した直後の状態。ブラウザは `pointerup` のあとに `click` を発火させるので、これを挟まないと運んだ先の要素が選択されてしまう
 * （移動は選択を変えない / #35）。閾値未満の動き（＝ただのクリック）ではここへ入らない。
 */
...
/**
 * 今の落とし方（docs/06-ui.md「キャンバス直接操作」の移動と、「編集操作の一覧」の挿入）。離したときに届く編集と、離す前の提示の両方を答える。
 *
 * ツリーへ挿すのと座標を置き直すのは**排他**なので直和で列挙する（並べて持つと「挿入位置と座標の両方がある」が型で書けてしまう）。
 *
 * 落とし先だけでなく**誰を**動かすかまで持つのは、運んでいるものと落とし方に成立しない組み合わせがあるため（パレットの雛形はまだ木に無いので座標を持てない）。
...
export const DropEdit = {
   ...
   *
   * 2 つを 1 つの型に畳まないのは、`RepositionTarget` が**書かれる座標**の対で、提示のためだけの画面上 px を混ぜないため。代わりに両方を作る場所を `useNodeDrag` の
   * 1 箇所（同じ `SideSnap.toSnapped` の戻り値）へ閉じている。線は丸める前の実測から引くので、書かれる座標との差は丸めのぶん（最大 0.5px / docs/06-ui.md）だけ残る。
   *
...
export const NodeDrag = {
  ...
  /**
   * 内側から外へ並べた候補のうち、最も内側の掴めるノードの名前。掴めるのは artboard 配下のノードだけ。
   *
   * artboard 自身の名前はどのツリーにも無いので（`Artboard.findNode` が探すのは `children`）ここでは引けず、ツリー内の移動先も持たない（artboard の並べ替えは
   * 別の操作 / docs/06-ui.md「編集操作の一覧」）。
   *
   * **artboard の背景を押したときにここが `none` を返すことに、キャンバスの掴み分けが載っている**（`ArtboardFrame` の `onPointerDown`）。引けるようにすると、
   * 背景を押しても artboard が動かなくなる。部品インスタンスの中身は木に無いので、そこを押すとインスタンス自身が掴まれる（#35）。
   *
  ...
  /**
   * 指を離す。既存ノードを運んでいたなら直後の `click` を飲み込む状態へ入り、動かしていなかったなら掴んでいない状態へ戻す。
   *
   * パレットの雛形を運んでいたときに飲み込む状態へ入らないのは、押した場所（パレットの行）と離した場所（キャンバス）が別の枝にあり、`click` が両者の共通の祖先へ
   * 飛ぶため。キャンバスの枠までは上がってこないので飲み込むものが無く、それでも `dropped` に入ると**次のクリック**を食べてしまう（選択が 1 回黙って消える）。
   */
