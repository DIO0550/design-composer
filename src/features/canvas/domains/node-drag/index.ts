 ...
 *
 * `dropped` は既存ノードを離した直後の状態。ブラウザは `pointerup` のあとに `click` を発
 * 火させるので、これを挟まないと運んだ先の要素が選択されてしまう（移動は選択を変えない /
 * #35）。閾値未満の動きではここへ入らない。
 */
...
export const DropEdit = {
   ...
   *
   * 2 つを 1 つの型に畳まないのは、`RepositionTarget` が**書かれる座標**の対で、提示のた
   * めだけの画面上 px を混ぜないため。代わりに両方を作る場所を `useNodeDrag` の 1 箇所
   * （同じ `SideSnap.toSnapped` の戻り値）へ閉じている。
   *
...
export const NodeDrag = {
   ...
   *
   * ツリー内の移動先も持たない（artboard の並べ替えは別の操作 / docs/06-ui.md「編集操作の
   * 一覧」）。部品インスタンスの中身は木に無いので、そこを押すとインスタンス自身が掴まれる
   * （#35）。
   *
   ...
   * なかったなら掴んでいない状態へ戻す。
   *
   * パレットの雛形を運んでいたときに飲み込む状態へ入らないのは、押した場所（パレットの行）
   * と離した場所（キャンバス）が別の枝にあり、`click` が両者の共通の祖先へ飛ぶため。
   *
   * キャンバスの枠までは上がってこないので飲み込むものが無く、それでも `dropped` に入ると
   * **次のクリック**を食べてしまう（選択が 1 回黙って消える）。
   */
