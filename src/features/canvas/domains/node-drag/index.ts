 ...
 * ものが変わるため直和で列挙する（「動かしていないのに落ちる先がある」を作れなくするた
 * め）。
 *
 * 掴むのはキャンバス上の既存ノードとパレットの雛形の 2 通りで、どちらを運んでいるかは
 * `DraggedNode` が持つ。2 つのドラッグに分けないのは、分けるとキャンバスが落とし先を 2
 * つ受け取ることになり「両方ドラッグ中」が型で書けてしまうため。
 *
 * `dropped` は既存ノードを離した直後の状態。ブラウザは `pointerup` のあとに `click` を
 * 発火させるので、これを挟まないと運んだ先の要素が選択されてしまう（移動は選択を変えな
 * い / #35）。閾値未満の動き（＝ただのクリック）ではここへ入らない。
 */
 ...
 *
 * 落とし先だけでなく**誰を**動かすかまで持つのは、運んでいるものと落とし方に成立しない
 * 組み合わせがあるため（パレットの雛形はまだ木に無いので座標を持てない）。分けて持つと
 * 「雛形を座標へ置き直す」が型で書け、受け取る側に捨てるだけの分岐が要る。
 */
...
export const DropEdit = {
   ...
   *
   * 2 つを 1 つの型に畳まないのは、`RepositionTarget` が**書かれる座標**の対で、提示の
   * ためだけの画面上 px を混ぜないため。代わりに両方を作る場所を `useNodeDrag` の 1 箇
   * 所（同じ `SideSnap.toSnapped` の戻り値）へ閉じている。線は丸める前の実測から引くの
   * で、書かれる座標との差は丸めのぶん（最大 0.5px / docs/06-ui.md）だけ残る。
   *
   *   @param name 置き直すノードの名前
   *   @param target 落とし先の親から見た座標と、運んでいる間のずらし量（ドキュメント上
   *   の px）
...
export const NodeDrag = {
  ...
  /**
   * 内側から外へ並べた候補のうち、最も内側の掴めるノードの名前。掴めるのは artboard 配
   * 下のノードだけ。
   *
   * artboard 自身の名前はどのツリーにも無いので（`Artboard.findNode` が探すのは
   * `children`）ここでは引けず、ツリー内の移動先も持たない（artboard の並べ替えは別の操
   * 作 / docs/06-ui.md「編集操作の一覧」）。
   *
   * **artboard の背景を押したときにここが `none` を返すことに、キャンバスの掴み分けが載
   * っている**（`ArtboardFrame` の `onPointerDown`）。引けるようにすると、背景を押して
   ...
   * なかったなら掴んでいない状態へ戻す。
   *
   * パレットの雛形を運んでいたときに飲み込む状態へ入らないのは、押した場所（パレットの
   * 行）と離した場所（キャンバス）が別の枝にあり、`click` が両者の共通の祖先へ飛ぶため。
   * キャンバスの枠までは上がってこないので飲み込むものが無く、それでも `dropped` に入る
   * と**次のクリック**を食べてしまう（選択が 1 回黙って消える）。
   */
