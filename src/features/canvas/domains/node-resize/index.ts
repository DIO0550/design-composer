 ...
 *
 * ハンドルの四角（`HandleAnchors`）とは別に、**右辺・下辺の全長が掴める**。描かれている四角だけを掴み口にすると、辺のどこでも掴めていたものが 10px の的に
 * なって操作性が落ちるため。
 *
 * **帯は角でも 1 軸のまま。** 2 本の帯が重なる角では `handles` の並び順で先にある軸（幅が固定なら幅）を掴む。角の付近を 2 軸にするには**どこからを角とみなすか**を
 ...
 *
 * `Placement` と呼ばないのは、このリポジトリの `placement` が「ノードが親の中でどう置かれるか」（`flow` / `absolute`。`domains/dcmp/placement`）を既に指しているため。
 *
 * `grip` はその箇所で何を変えられるかで、掴めない箇所では `none`。右下の角だけが `both` なのは、**原点（左上）が動かない**ので幅と高さを増やすだけで済むため
 * （残る 3 隅と左辺・上辺は反対側の辺を留める補正が要り、既定の `placement: "flow"` のノードは座標を持たないので補正できない / #371）。
 */
