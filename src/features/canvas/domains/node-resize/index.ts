 ...
 * が固定なら幅）を掴む。角の付近を 2 軸にするには**どこからを角とみなすか**を決める必要
 * があり、それは掴み口全体の設計そのものになるので #371 に残している。結果として、角の
 * 四角の外側・帯の内側を押すと 1 軸しか変わらない。
 */
 ...
 * ・上辺は反対側の辺を留める補正が要り、既定の `placement: "flow"` のノードは座標を持た
 * ないので補正できない / #371）。
 */
