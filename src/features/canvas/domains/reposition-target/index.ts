...
/**
 * 絶対配置のノードを運んで離したときの行き先（#388）。ツリーへ落とすときの `DropTarget` と対になる、座標で動かすときの落とし先。
 *
 * 書かれる座標（`to`）と、運んでいる間に見た目をずらす量（`offset`）を対で持つ。親を付け替えると座標の原点が変わるので、**画面上の位置を変えない**ために
 * 書かれる座標だけが原点のずれを含み、見た目のずらし量は運んだ分のままになる。
 *
 * 2 つは同じ 1 回のドラッグの別々の見え方で、別々に組むと食い違うため `create` からしか作らない。
 */
