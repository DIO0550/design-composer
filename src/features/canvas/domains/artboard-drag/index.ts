 ...
 * の artboard の移動）。掴んだものを持つのは離すまで、ポインタの位置を持つのは動かして
 * いる間だけ、と状態ごとに持つものが変わるので直和で列挙する。
 *
 * ノードのドラッグ（`NodeDrag`）と別の状態機械にしているのは、artboard の移動が**ツリー
 * の操作ではない**ため（同じ状態機械へ載せると「artboard をツリーへ移動する」が型で書け
 * てしまう）。
 *
 * `NodeDrag` / `NodeResize` が持つ「離した直後」の状態は持たない。**どちらの掴み口にも
 * 飲み込む相手が居ない**ため（見出しは枠の兄弟なので `click` が枠へ上がってこず、背景は
 * 運んだあと `click` そのものが出ない。後者は Chromium で実測）。飲み込むものが無いのに
 * その状態へ入ると、**次のクリックを食べる**。
 *
 * `click` を出す実装系のために飲み込む案を採らないのは、**背景を押すのが元から選択の操
 * 作**で、運んだ artboard が選ばれるだけで害が無いため。
 */
