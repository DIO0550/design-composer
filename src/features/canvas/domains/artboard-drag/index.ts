 ...
 *
 * `NodeDrag` / `NodeResize` が持つ「離した直後」の状態は持たない。掴み口（見出し）は
 * 選択を受ける枠の**兄弟**にあるので、ドラッグ直後の `click` は共通の祖先どまりで枠へは
 * 上がってこない。飲み込むものが無いのにその状態へ入ると、**次のクリックを食べる**
 * （`NodeDrag.release` が雛形のドラッグについて書いているのと同じ形）。
 */
