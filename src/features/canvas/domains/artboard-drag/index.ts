...
/**
 * キャンバス上で artboard を掴んでから離すまでの状態（docs/06-ui.md「キャンバス直接操作」の artboard の移動）。掴んだものを持つのは離すまで、ポインタの位置を
 * 持つのは動かしている間だけ、と状態ごとに持つものが変わるので直和で列挙する。
 *
 * ノードのドラッグ（`NodeDrag`）と別の状態機械にしているのは、artboard の移動が**ツリーの操作ではない**ため（同じ状態機械へ載せると「artboard をツリーへ移動
 * する」が型で書けてしまう）。
 *
 * `NodeDrag` / `NodeResize` が持つ「離した直後」の状態は持たない。**どちらの掴み口にも飲み込む相手が居ない**ため（見出しは枠の兄弟なので `click` が枠へ上がって
 * こず、背景は運んだあと `click` そのものが出ない。後者は Chromium で実測）。飲み込むものが無いのにその状態へ入ると、**次のクリックを食べる**。
 *
 * `click` を出す実装系のために飲み込む案を採らないのは、**背景を押すのが元から選択の操作**で、運んだ artboard が選ばれるだけで害が無いため。
 */
