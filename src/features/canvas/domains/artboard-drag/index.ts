 ...
 *
 * `NodeDrag` / `NodeResize` が持つ「離した直後」の状態は持たない。掴み口はどちらも
 * 飲み込む相手を持たないため。見出しから運んだときの `click` は枠の**兄弟**どまりで
 * 枠へ上がってこず、背景から運んだときはそもそも `click` が出ない（Chromium で実測。
 * 閾値を越えて動かすと押した位置と離した位置が離れるため）。飲み込むものが無いのに
 * その状態へ入ると、**次のクリックを食べる**（`NodeDrag.release` が雛形のドラッグに
 * ついて書いているのと同じ形）。
 *
 * 仮に届く実装系があっても飲み込まない。**背景を押すのは元から選択の操作**なので、
 * 運んだ artboard が選ばれるだけで害が無い。
 */
