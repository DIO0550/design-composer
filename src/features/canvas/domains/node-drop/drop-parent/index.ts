...
export const DropParent = {
   ...
   *   - 運んでいるものが占めている名前（入れるとツリーが壊れる。占めている名前は運んで
   *   いるものの種別で変わるので `DraggedNode` が答える）
   *
   ...
   *
   * 運んでいるノードの上を通ったときにその親が選ばれるのは、外へ辿った結果であって既定
   * 値へ倒しているわけではない（元の位置へ戻すのは正当な移動）。
   *
   *   @param document 名前の引き先になるドキュメント
   *   @param dragged 運んでいるもの
   *   @param names ポインタの下から根へ向かう順の候補
   *   @returns 受け入れられる最も内側の親。候補が1つも受け入れられない場合と、  木に無
   *   いノードを運んでいる場合は `none`
   */
  innermost(
    document: DesignDocument,
