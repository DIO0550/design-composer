 ...
 *
 * ドキュメント（source of truth）には含めない実行時だけの状態なので、編集画面の状態ではなくキャンバスの中に閉じて持つ（docs/01「作業スペースの見た目
 * （配置・ズーム等）は source of truth に含めない」）。
 *
 * `dragFrom` はドラッグ中だけ意味を持つ**直前のポインタ位置**で、移動量はここからの差分で決まる。ドラッグしていないのに開始位置だけが残る矛盾した状態を
 * 作れないよう `Option` で表す。
 */
...
export const CanvasView = {
  ...
  /**
   * 指した矩形が画面に収まる倍率と位置にする（Figma の Zoom to fit / Zoom to selection）。ここで言う「収める」は**指した矩形が画面の中に入る倍率と位置にする
   * こと**で、CSS の `fit-content` とは関係しない。
   *
   * 受け取るのはどちらも**実測した画面上の矩形**（client 座標）。`target` は倍率と位置が効いた状態で描かれているので今の見え方から割り戻す（そのため押した時点の
   * 倍率・位置に依らず同じ結果になる）。
   *
   * `viewport` に渡すのは**キャンバスの土台**（`canvas-surface`）の矩形で、その左上が `transform` の原点であることを前提にしている。この前提が崩れると絵だけがずれる。
   *
   * ドキュメント側の座標を受け取らないのは、ノードの大きさを決めるのがブラウザのレイアウトで、選択に合わせる側の矩形はドキュメントからは出せないため。
   *
   * @param view 割り戻しに使う今の見え方
