 ...
 * artboard を 1 枚足すボタン。
 *
 * `PrimitiveInsertButton` に相乗りさせないのは、プリミティブが選択位置へ挿すので挿せる
 * 位置が無ければ押せないのに対し、artboard は選択に依らず常に押せるため（`Artboards` の
 * 見出しの `+` と同じ。押せない理由の `title` も要らない）。
 *
 * 字面を手書きせず `TypeGlyph` を使うので、`#` は UI 案のツールバーの灰（`#5c5c5c`）で
 * はなく種別色の青になる。`□` / `T` が既に同じ乖離を抱えており（#112 の単位で扱う）、こ
 * こだけ寄せると同じ列で色の決まり方が 2 通りになる。**この色はテストでは落ちない** —
 ...
 *
 * UI 案が先頭に置くポインタは持たない（ツールモードの概念が無い）。**`#` を先頭に置く**
 * のはそのためで、UI 案の並び（`ポインタ → # → □ → T`）のうち実装する 4 つの相対順序を
 * 保つと `#` が先頭になる。`◆` は**ボタンではなく状態表示**（#203）。プリミティブの並び
 * は `PrimitiveTypes` から作る（スキーマと二重管理しない）。
 *
 * `#` を「今のツール」のトグルにしないのは、この実装では `□` / `T` が既に即時挿入のボタ
 * ンで、`#` だけをモードにすると 1 列の中で操作モデルが 2 つに割れるため。
 *
 * 押せる条件を artboard について持たないのは、**ファイルが不正な間はこのツールバー自体
 * が出ない**ため（出し分けはキャンバス下端に積む器が持つ / #128）。
 *
 * 影と丸みはこの部品が持つが、**位置は持たない**。キャンバス下端にはエラー一覧も並ぶた
 * め、順序は両方を積む器（`CanvasDockStack`）が決める（#128）。**形を落としてもテストは
 * 落ちない** — happy-dom は Tailwind を解決しない。気づく手段は Storybook の視覚差分だ
 * け。
 */
