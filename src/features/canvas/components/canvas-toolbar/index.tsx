 ...
 * 操作に別の名前を与えない）。共有の定数にしないのは、表示の綴りのために feature 間へ辺を
 * 張ることになるため（片方だけ変えると、同じ操作が 2 つの名前で読み上げられる）。
 */
 ...
 *
 * 押せる条件を artboard について持たないのは、**ファイルが不正な間はこのツールバー自体が
 * 出ない**ため（#128）。影と丸みは持つが**位置は持たない**（下端にはエラー一覧も並ぶので、
 * 順序は器 `CanvasDockStack` が決める）。
 *
