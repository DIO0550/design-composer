...
/**
 * artboard を 1 枚足すボタン。`PrimitiveInsertButton` に相乗りさせないのは、プリミティ
 * ブが選択位置へ挿すので挿せる位置が無ければ押せないのに対し、artboard は選択に依らず常
 * に押せるため。
 *
 * 字面を `TypeGlyph` から採るので、`#` は UI 案のツールバーの灰（`#5c5c5c`）ではなく種
 * 別色の青になる。`□` / `T` が既に同じ乖離を抱えており（#112 の単位で扱う）、ここだけ寄
 * せると同じ列で色の決まり方が 2 通りになる。**この色はテストでは落ちない** — 気づく手
 * 段は視覚差分だけ。
 */
 ...
 * ボタンではなく状態表示** / #203）。
 *
 * UI 案が先頭に置くポインタは持たない（ツールモードの概念が無い）ので `#` が先頭になる—
 * UI 案の並び（`ポインタ → # → □ → T`）のうち実装する 4 つの相対順序を保った結果。`#`
 * をモードのトグルにしないのも、`□` / `T` が即時挿入で 1 列の操作モデルが割れるため。
 *
 * 押せる条件を artboard について持たないのは、**ファイルが不正な間はこのツールバー自体
 * が出ない**ため（#128）。影と丸みは持つが**位置は持たない** — 下端にはエラー一覧も並ぶ
 * ので順序は器（`CanvasDockStack`）が決める。**形を落としてもテストは落ちない**（happy-dom
 * は Tailwind を解決しない。気づく手段は視覚差分だけ）。
 */
