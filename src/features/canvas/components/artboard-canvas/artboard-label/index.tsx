 ...
 *
 * 名前が青く太くなるのは「今ツリーが映している 1 枚」のとき（#184）。UI 案で色が付いているのは 10 行中 1 行だけで、その画面では artboard 自身ではなく
 * 配下のノードが選択されている一方、`Artboards` 一覧が `login` をハイライトしている唯一の画面でもあるので、青が指しているのは選択ではないと読んだ
 * （選ばれているか（`DocumentSelection.isSelected`）で色を決めると、UI 案の唯一の色付きを再現できない）。
 *
 * 大きさの綴りを `artboard-list` と共通化しないのは、UI 案がツリー側で `720×900`、キャンバス側で `720 × 900` と空白の有無を変えているため。
 *
 * 見出しは artboard を動かす掴み口の 1 つ（docs/06-ui.md「キャンバス直接操作」。もう 1 つは枠の背景）。**子が全面を覆う artboard**では背景を押せない
 * ので残している。
 *
