 ...
 * 掴めるものに出すカーソル。
 *
 * export しているのは、掴んでいる間のカーソルを器（`ArtboardCanvas`）が出すため（ハンドルはそのあいだポインタを通すので自分では出せず、別々に綴ると押す前と
 * 押している間でカーソルが変わってしまう）。
 *
 * 掴んだ箇所を受け取って斜めの向きを出し分けていないのは、2 軸を掴めるのが右下の角だけで `nwse-resize` に決まり、向きを引数で受けても**どの入力でも同じ枝しか
 * 通らない**ため。右上・左下（`nesw-resize`）を掴めるようにする回に、そのとき初めて箇所を持ち込む。
 *
 * @param grip 掴めるもの
 ...
 *
 * `<style>` の差し込みではなく実要素で描くのは、**掴める位置にだけカーソルを出す**ため（CSS の `cursor` は箱ごとにしか効かないので、擬似要素 1 つに背景で
 * 8 個描く形では要素全体が同じカーソルになる）。実要素にしたことで、辺をまたぐ配置と UI 案の角丸（`border-radius:1px`）も同時に戻せている。
 *
 * 掴んでいる間は**全部をポインタに対して透明にする**。移動と解放を受けるのはキャンバスの器（`canvas-content`）で、ハンドルが不透明のままだと追いかけてきた
 * ハンドルにポインタが乗った瞬間に器から離脱したことになり、取り消し（`onPointerLeave`）が飛ぶ。
 *
 * 座標は器からの相対（`CanvasBounds.relativeTo`）。`position:fixed` で client 座標をそのまま使う `DropMarker` と違うのは、ハンドルが**選択している間ずっと出る**
 * ため（`fixed` は器の `overflow` を抜けるので、選択したままパンして対象を画面外へ出すと左右のペインの上にハンドルが残る）。
 */
