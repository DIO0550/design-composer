...
/**
 * キャンバス（docs/06-ui.md「画面構成」）。artboard をキャンバス上の座標へ置き、コンパイル結果（実 HTML / CSS）をレンダリングする。ズーム / パンは
 * 非永続の view state で、ドキュメントには保存しない。
 *
 * 表示（倍率・位置）を自分で持たず受け取るのは、倍率の操作が上部バーへ移り、キャンバスと上部バーが同じ 1 つの表示を見る必要があるため（#134）。
 *
 * props が 10 個あるが Composition へは割っていない。関心は「キャンバス」1 つで、中身を子要素として受け取る形にはできない（描くものはコンパイル結果の
 * HTML で、呼び出し側が組み立てられない）。`EditorState` を丸ごと受けると feature として切り出せない（#256）。
 *
 * 中央ペインの凍結（ハンドルの抑止・`inert`・スクリム）はここが自分で出す。左右のペインと違い器の `EditorLayout` は中央に淡色も `inert` も付けない
 * （映っているものは最後に正常だった表示なので、見る操作だけは残す）。
 *
 * `selection` と `tokenSelection` がそれぞれドキュメントを持つが、束ねる型は作らない。トークンの対を単独で受けている `TokenList` / `TokenEditor` と流儀が
 * 割れるため（同じドキュメントを指すことは、両方を 1 つの状態から作る呼び出し側が保つ）。
 *
 * ツリー内の移動 / 挿入のドラッグを自分で持たず受け取るのは、掴む場所がパレット（左ペイン）にもあり、状態を両方の親が持つため。
 */
