 ...
 * す。
 *
 * `pointer-events:none` が要るのは、これが無いとポインタの下にあるのが運んでいるノード
 * 自身になり、`DropParent.innermost` が元の親しか答えないため（＝同じ親の中にある兄弟の
 * Box へ入れられない）。親が中身を切り取る artboard のときだけは付け替えが効いてしまい、
 * **親の種類で挙動が変わる**。
 *
