 ...
 *
 * 先頭の見本を出すのは色だけ。UI 案が描いているのも色の 14×14 のチップだけで、無い絵を思いつきで足さない
 * （rules/ui-verification.md）。
 *
 * UI 案に無い枠線を足しているのは、白や薄い色のトークンが白い帯に溶けて見本が消えるため（UI 案の見本は `#111827` の
 * 1 例だけで、この場合が出ていない）。
 *
 * `components/color-swatch` を使わないのは、UI 案が一覧の 12px の四角とこの帯の 14px の角丸チップを描き分けており、寄せると
 * どちらかが UI 案から離れるため。
 */
...
/**
 * 打っている途中の文字列を持ち、確定したときだけ外へ渡す入力欄。確定は入力欄を離れたときと Enter。
 *
 * 1 打鍵ごとに渡すと、確定形だけを受け付ける値（kebab-case の名前・数値）では途中の文字列が弾かれて打ち続けられない
 * （`primary-` が弾かれると `-` の次を打てない）。
 *
 * 外の値が変わったときの取り直しは `key` で行う（呼び出し側が現在値を key に混ぜる）。確定が通らなかった入力では外の値が
 * 変わらず key も変わらないので、下書きが残ってそのまま打ち続けられる。
 *
 * ただし `type="number"` の欄は下書きが数値として読めない間ブラウザが表示を空にするので、画面に文字列として残るのは
 * `type="text"` の欄だけ。
 */
function DraftField({
  id,
