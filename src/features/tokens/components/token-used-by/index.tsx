 ...
 * 参照元に当たる型アイコン。
 *
 * 種別の語彙は `TypeGlyph` が受けるものに合わせる（ツリーの行も同じ語彙でアイコンを描く）。
 * 部品定義は選択の対象ではないが、指しているものが部品である点はインスタンスと同じなの
 * で同じアイコンになる。
 *
...
/**
 * 選択中のトークンの参照元（UI 案 docs/Design Composer.html の Tokens 画面の `Used by`）。
 *
 ...
 *
 * 件数は表示している行数ではなく参照元の総数で、`+ N more` はその差。押せる形にしていない
 * のは UI 案がこれを灰色の文字として描き `cursor:pointer` を持たせていないためで、全参照元
 * をどこで見せるかはキャンバスとの連動が持つ。
 *
