 ...
 * 参照元に当たる型アイコン。
 *
 * 種別の語彙は `TypeGlyph` が受けるものに合わせる（ツリーの行も同じ語彙でアイコンを描く）。部品定義は選択の対象ではないが、
 * 指しているものが部品である点はインスタンスと同じなので同じアイコンになる。
 *
 * `switch` に `default` を置かず戻り値を `TypeGlyphKind`（`undefined` を含まない）にしているので、参照元の種類を足して
 * ここを足し忘れるとコンパイルエラーになる。
 *
 ...
 *
 * 参照が 0 件でも見出しと件数は出す。使われていないことは削除の判断材料なので、節ごと消すと「使われていない」と「まだ調べて
 * いない」が同じ見た目になる。枠は行があるときだけ出す（中身の無い枠を置かない）。
 *
 * 数えるのはデフォルト解決後の参照なので、0 件は「消してもエラーにならない」と読んでよい（artboard だけに残る例外は
 * `TokenReferrer.collectInArtboard` の doc が持つ）。件数は表示している行数ではなく参照元の総数で、`+ N more` はその差。
 *
 * `+ N more` を押せる形にしていないのは、UI 案がこれを灰色の文字として描き `cursor:pointer` を持たせていないため。全参照元を
 * どこで見せるかは、キャンバスとの連動（#147）が持つ。
 *
