...
/**
 * キャンバスの破線が何を指しているかを示す帯（UI 案 docs/Design Composer.html の Tokens 画面。キャンバス下部に浮く）。
 *
 * 破線が 1 本も無いときは出さない。この帯が伝えるのは「今どれが破線になっているか」なので、破線が無いまま出しても指す相手が
 * いない（0 件でも件数を出す `Used by` は、使われていないこと自体が削除の判断材料になる点が違う）。
 *
 * 左寄せ（`self-start`）を器ではなくここが持つのは、UI 案でツールバーが中央、この帯だけが左に寄っているため（器の既定を
 * 変えると他の 2 つまで動く）。見本を出すのは色だけで、UI 案が描いているのも `#111827` の四角 1 例。
 *
 * `token-control` の `TokenPreview` を使わないのは、使うと `token-list` の `PreviewSlot` と 4 枝すべてが重なるため。
 *
 * 飛び先は破線の先頭。UI 案がリンクを 1 本しか描いていないので複数ある参照元から 1 つ選ぶことになり、並びは既に
 * `collectCanvasReferrerNames` が決めている（#209）。
 *
...
export function TokenDashedNodes({
   ...
   * 数え終えた名前を props で受け取らない。受け取ると、名前とトークンが
