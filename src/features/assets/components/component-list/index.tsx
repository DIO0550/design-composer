 ...
 * `ComponentAsset.isUnused` が答え、こちらは `unused` と `×N` のどちらを書くかだけを持
 * つ）。
 *
 * 絞り込みは行わない。何を出すかは検索欄を持つ `AssetsPanel` が決める。1 件も無いときに
 * 「部品がありません」と言わないのは、絞り込みで 0 件になっただけかもしれないため。
 *
 * 部品は選択の対象にしない（選択できるのは artboard とその配下のノードだけ）。選択中の
 * インスタンスの元になっている行は、選択ではなく**出どころ**として強調する（UI 案の
 * `source of selection`）。
 */
