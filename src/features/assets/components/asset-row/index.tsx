...
/**
 * パレット（`Assets`）の 1 行。型アイコンと名前を出し、掴んでキャンバスへ落とす起点になる。押しても何も挿さらない
 * （UI 案は `Assets` を browse-only とし、挿入をドラッグだけの入口にしている / #203）。
 *
 * `Primitives` と `Components` の両方がこの形を使うのは、掴んでいるかの判定・掴んでいる行の見え方が同じで、片方だけ
 * 直ると食い違うため。違うのは右端に何を出すかだけなので `children` で受ける。
 *
 * 掴んでいる間は掴んでいることを優先して出す。出どころは選択が変わらない限り残るので、両方出すとどちらの意味の色か
 * 読めなくなる。
 *
