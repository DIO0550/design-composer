 ...
 *
 * 押す前と打っている最中を `Option<string>` 1 つで持つ（`none` = まだ押していない / `some` = 下書き）。真偽値と文字列に
 * 分けると「打っていないのに下書きがある」が表現できてしまう。
 *
 * 作れたあとに下書きを捨てる処理を持たないのは、作れた時点で選択がインスタンスに変わりこの部品ごと消えるため。選択を
 * 別のものへ移したときの取り直しは、呼び出し側が `key` に元の名前を混ぜて行う。
 *
