...
export type NestedRow = Readonly<{
   ...
   *
   * `children` や render prop ではなく行データに JSX を積むのは、木が再帰するので `children` では
   * 入れ子を表せず、render prop にすると呼び出し側が名前から元を引き直す対応表を持つため。
   */
  content: ReactNode;
  /**
...
/**
 * 同じ親の中での位置。親の名前と index は片方だけでは位置が決まらないため 1 つの型にまとめる。
 *
 * 同じ構造の `ChildPosition`（`src/domains/dcmp/child-position`）を綴り直しているのは、横断層から
 * `domains/` を import できないため。渡せることは `document-tree.type.test.ts` が型で固定している。
 */
 ...
 *
 * **ドラッグの状態をこの階層ごとに持つ。** 1 つの `<ul>` = 1 つの親の子の並びなので、掴んだ群だけが
 * 落ちる先を持ち、別の親の行の上で離しても移動が起きない（`onReorder` が並びの外を指さない）。
 *
...
/**
 * 入れ子の行を並べ、枝の開閉と、同じ親の中での並べ替えができる器。行に何を描くかは持たず、
 * 字下げ・開閉の列と、行がどの並びのどこにいるかだけを持つ。
 *
 * 並べ替えは行を掴んで運ぶ（`useReorderDrag`）。掴む口を配るのも、移動先を「同じ親の中の位置」
 * として読み替えるのも階層ごとの `RowList` で、ここが渡すのは移動を受け取る口だけ。
 *
 * 畳んだ**側**の名前を持つので、初めて描いたときは全部が開いた状態になり、後から増えた行が
 * 畳まれた状態で現れることもない。
 *
