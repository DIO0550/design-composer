...
/**
 * 右ペインの幅。編集画面のグリッド（`EditorLayout` の `grid-cols-[19rem_1fr_18rem]`）の 3 列目の
 * 写しで、両者が揃っていることは `editor-layout.pane-width.test.tsx` が固定する。
 *
 * 任意値で書くのはそのテストが文字列として突き合わせるため。グリッドの列指定から組み立てない
 * のは、Tailwind の JIT がソース中のリテラルしか拾わないため。
 *
 * この 18rem は UI 案（docs/Design Composer.html は右ペインを 5 画面とも 264px と描く）とずれて
 * いるが、それはグリッド側に
 * 元からある乖離で、ここはその写し。
 */
...
/**
 * ストーリーの中で右ペインの殻（`EditorLayout.RightPane` とそれを載せるグリッドの列）の代わりに
 ...
 * 左ペインのストーリーがこれを着ると 1rem ずれる（左ペインの枠は #304）。
 *
