 ...
 *
 * ツリービュー・artboard の一覧・エディタ画面のどれもが同じ読み方をするため、feature をまたいで
 * 読めるよう横断層に置く（持っているのは「`aria-current` を名乗るボタンが行」という汎用 UI の
 * 知識だけ）。行は名前のほかに型アイコンと補助情報も出すため、名前は表示文字列ではなく読み上げ名
 * （`aria-label`）から読む。
 */

/**
 * 行かどうかは `aria-current` を持つかで見る。一覧には行以外のボタン（並べ替え・開閉）も並ぶが、
 * `aria-current` を名乗るのは行だけなので、ボタンが増えても判定は変わらない。
 *
 * 危ないのは逆で、**行ではないのに `aria-current` を名乗るボタン**を足すと、ここが黙ってそれを
 * 行として拾う（テストは落ちない）。そうなったら行そのものに役割を宣言して引く形へ変えること。
 */
