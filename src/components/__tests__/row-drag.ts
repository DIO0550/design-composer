 ...
 * （`features/editor/features/sidebar`）・編集画面の通し（`features/editor`）のどれもが同じ引き方を
 * するため、横断層に置いて共有する。
 *
 ...
 * 名前で行の枠を引く。
 *
 * ツリーの `<li>` は配下の並びまで包んでいるので、そちらに張ると子の行を押したときに親を掴
 * んでしまう（pointerdown はバブルする）。artboard の一覧は `<li>` が行そのものなので、ど
 * ちらでも同じ要素に行き着く。
 *
