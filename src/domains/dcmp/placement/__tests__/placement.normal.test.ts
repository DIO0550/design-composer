...
test("フローに参加している配置は、絶対配置として取り出せない", () => {
  expect(
    Placement.absoluteFromProps({ placement: "flow", x: 40, y: 24 }),
  ).toEqual(Option.none);
});

test("座標で置かれる配置は、絶対配置として取り出せる", () => {
  expect(
    Placement.absoluteFromProps({ placement: "absolute", x: 40, y: 24 }),
  ).toEqual(Option.some({ mode: "absolute", x: 40, y: 24 }));
});
