...
export const ComponentSet = {
   ...
   *
   * 数えるのは `outsideNodes`（部品の外側にある木）にある参照ノードと、**部品定義の中にある参照ノードの
   * 両方**。部品 A が部品 B を含んでいれば B は使われているので、外側だけを見ると「どこからも使われて
   * いない」と読める部品が出てしまう。
   *
   * `Node.collectRefs` は参照ノードで止まり参照先を展開しないため、部品同士が循環していても各定義を
   * 1 回ずつ見るだけで終わる。定義の無い名前への参照（dangling）はどの部品の数にも入らない。
   */
  assets(
    components: ComponentSet,
