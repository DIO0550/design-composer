 ...
 *
 * 座標は**親の左上を原点とする値**なので、親の名前と対でなければ画面上の位置が決まらない（同じ
 * `x` / `y` でも親が違えば別の場所を指す）。片方だけを持ち回れないよう 1 つの型にする。
 *
 * `ChildPosition` と対になる型で、**あちらは並びの位置（何番目の子か）、こちらは親の中の座標**。親の
 * 名前が子を受け入れられる相手かどうかはここでは決めない（`DesignDocument.reposition` が失敗として答える）。
 */
