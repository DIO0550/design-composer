...
export { GradientToken, type LinearGradientValue } from "./gradient";
export {
  type BoxShadowValue,
  type ShadowField,
  ShadowFieldEdit,
...
export const Token = {
   ...
   *
   * 影とグラデーションも通すのは、どちらも中に生 hex を持つため
   * (docs/04-tokens.md「shadows」「gradients」)。色の種別だけを通すと、影の中の hex と
   * stop の hex が大文字のまま保存される。
   */
...
function withoutToken(tokens: TokenSet, ref: TokenRef): TokenSet {
  switch (ref.kind) {
    ...
    case "gradients":
      return { ...tokens, gradients: withoutName(tokens.gradients, ref.name) };
  }
}
