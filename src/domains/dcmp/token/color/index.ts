 ...
 * alpha を持たない色(`#rrggbb`)。
 *
 * `ColorToken` と分けるのは、alpha を持てるかどうかで**できることが変わる**ため。カラーピッカー
 * (`input[type=color]`)が扱えるのはこの形だけで、同じ型にすると一方を他方のつもりで渡しても型が通ってしまう。
 *
 * 生成は `create` だけに閉じる。`#` から始まる形を型に出しているので、素の `string` は代入できない。
 */
...
export const ColorToken = {
  ...
  /**
   * hex として読める値を正規形(小文字・不透明なら6桁)へ倒す。hex でない値は正規形が定義できないので、意味を
   * 変えずそのまま返す(不正値としての報告はバリデーションの担当)。
   *
   * `ff` を落とすのは、`#111827` と `#111827ff` が同値異表記で、docs/04-tokens.md「値の形式」が正規形を 1 つに
   * 保つと明文で決めているため。
   */
