 ...
 * alpha を持たない色(`#rrggbb`)。
 *
 * カラーピッカー (`input[type=color]`)が扱えるのはこの形だけで、同じ型にすると一方を他方の
 * つもりで渡しても型が通ってしまう。
 *
 * 生成は `create` だけに閉じる。`#` から始まる形を型に出しているので、素の `string` は
 * 代入できない。
 */
...
/**
 * alpha の2桁を正規形（小文字）で返す。持っていなければ空文字（不透明を表す桁を足さない）。
 *
 * hex の綴りを別の正規表現で書き直すと、片方だけ直したときに気づけないので既存のパターン
 * の任意キャプチャから引く。
 *
...
export const ColorToken = {
  ...
  /**
   * hex として読める値を正規形(小文字・不透明なら6桁)へ倒す。hex でない値は正規形が定義
   * できないので、意味を変えずそのまま返す(不正値としての報告はバリデーションの担当)。
   *
  ...
  alphaPercentOf(color: ColorToken): number {
    ...
    return NumberEx.round((byte / OpaqueAlphaByte) * 100, AlphaPercentDigits);
  },
