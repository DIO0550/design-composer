 ...
 *
 * 版ごとの型と JSON 表現は版のフォルダ（`v1/`）が持つ。major を上げるときは隣に `v2/` を作ってここを
 * 差し替え、旧版のフォルダは残す（旧版の型が残ることで、マイグレーションが「どの形から どの形へ」を
 * 型で書ける）。ここを版の直和にしないのは、消費側に版の分岐を強いないため。
 */
...
/**
 * 大きさが変わった artboard / ノードの、直下の絶対配置の子を追従させる（docs/03「配置の指定」）。
 *
 * 子への書き込みを `applyPropEdit` へ戻すので、**長さが変わった子は自分の子の追従を自分で引き起こす**
 * （明示的な再帰を書かない）。「どの prop を編集したか」で要否を決めないのは、`width` を消して `hug` へ
 * 戻す編集のように prop 名だけでは長さが変わったかを判定できないため。
 *
...
export const DesignDocument = {
   ...
   *
   * 直下だけで止めるのは、キャンバスから選べる階層がそこだから（掘るのはダブルクリックの担当 /
   * docs/06-ui.md「キャンバスのクリックが選ぶ階層」）。孫まで集めると、範囲で払ったときにクリックとは
   * 違う階層が選ばれる。部品定義の中にある参照ノードを見ないのは、キャンバスに描かれてもドキュメントの
   * 木には無く、選択の対象にならないため。
   *
   * @param document 走査するドキュメント
  ...
  /**
   * 名前で指したノードが今どの親の中のどこに置かれているか。**座標で動かせるものだけ**が答えを持つ。
   *
   * 絶対配置だけを答えるのは、消費側（キャンバスのドラッグ）が知りたいのが「このノードは座標で動かせるか、
   * 動かせるなら今どこか」だから。座標と親を別々に答えないのは、座標が親の左上を原点とする値で、**片方
   * だけでは位置が決まらない**ため（`ChildPlacement`）。
   *
   * @param document 引き先になるドキュメント
   * @param name 置かれている場所を知りたいノードの名前
   * @returns 今いる親と、その親から見た座標。木に無い名前 / 部品インスタンス（props を持たない）/ フロー /
   *   座標が数値でないとき / 親を持たない artboard 自身は `none`
   */
  childPlacementOf(
    document: DesignDocument,
    name: string,
  ...
  /**
   * 名前で指した artboard またはノードの prop を書き換える（docs/06-ui.md「編集操作の一覧」の props 編集）。
   * 名前は単一名前空間なので、artboard とノードのどちらを相手にするかは名前で決まる。
   *
   * 大きさが変わったときは、直下の絶対配置の子をここで追従させる（`withResizeFollowUp`）。`resize` の
   * **ノード経路**にフックを置かないのは、そこがこの入口を通るため（両方に置くと子が 2 度動く）。
   */
  applyPropEdit(
    document: DesignDocument,
    name: string,
  ...
  /**
   * 名前で指した artboard またはノードの大きさを変える（docs/06-ui.md「キャンバス直接操作」のリサイズ
   * ハンドル）。長さの持ち主が artboard とノードで違うため、名前で相手を決めてから書き込み先を分ける。
   *
   * ノード側でモードが `fixed` かどうかは見ない（ハンドルを出す軸を決めるのはキャンバス側の役目）。絶対
   * 配置の子の追従は artboard の経路だけここで起こす。ノードの経路は `applyPropEdit` を通る。
   */
  resize(
    document: DesignDocument,
    name: string,
  ...
  /**
   * 名前で指したノードを、親の中の別の座標へ置き直す（docs/06-ui.md「キャンバス直接操作」の移動のうち、
   * 絶対配置のノードの分）。
   *
   * 座標の 2 prop を 1 回の呼び出しで書くのは、ドラッグ 1 回が undo 1 回で戻るようにするため。
   *
   * artboard を相手にしないのは、artboard が親 Box の中ではなくキャンバスの並びに置かれるため。今いる位置
   * （`findChildPosition`）で相手を確かめるので、どの親の子でもない artboard はここで弾かれる（素通しすると
   * artboard の props に効かない `x` / `y` が黙って書かれる）。
   *
   * 指した親が今の親と違えば、**その親の末尾の子へ移してから**座標を書く（#388）。絶対配置の兄弟に並び順の
   * 意味が薄く、元の index を持ち込むと落とし先の子の数によっては範囲外になるため。
   *
   * @param document 書き換える対象を含むドキュメント
   * @param name 置き直すノードの名前
   * @param to 置き直したあとの親と、その親から見た座標
   * @returns 親と座標を書き換えたドキュメント。その名前のノードが無い（artboard の名前もノードではない）
   *   なら失敗。指した親が子を受け入れられない（無い名前・Text・参照ノード）ときと、指した親が自分自身か
   *   自分の子孫のとき（`moveNode` の `move-into-descendant`）も失敗
   */
  reposition(
    document: DesignDocument,
    name: string,
  ...
  /**
   * 単一名前空間の名前で指したものを取り除く（docs/06-ui.md「編集操作の一覧」の削除と artboard 操作）。
   * artboard ならその 1 枚を配下ごと、そうでなければノードをサブツリーごと取り除く。
   *
   * 振り分けをここに置くのは、名前だけでは artboard かノードかが決まらず、どのドキュメントの中の名前かで
   * 初めて引けるため。呼び出し側で分けると「選んでいるものが artboard か」の判定が features 層へ出る。
   *
   * @param document 取り除く先のドキュメント
   * @param name 取り除きたい artboard / ノードの名前
   * @returns 取り除いたドキュメント。どちらにも無い名前は `node-not-found`（artboard でなければノードとして
   *   扱うため）
   */
  remove(
    document: DesignDocument,
  ...
  /**
   * そのトークンを参照している箇所をすべて集める（UI 案 docs/Design Composer.html の `Used by` / #127）。
   *
   * artboard の中（キャンバス上のもの）を先に、部品定義の中を後に並べる。一覧は先頭の数件しか出さないので、
   * 選択やキャンバスから指し示せるものを先に見せる（UI 案は両者を交互に並べているが、それを再現できる
   * 大域順序が無いため規則で決めている）。
   *
   * トークンが実在するかは見ない。「その参照を指している prop はどれか」に答えるので、宙に浮いた参照
   * （dangling）も同じ関数で数えられる（存在の確認は `TokenSet.has` の担当）。
   */
