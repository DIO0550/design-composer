...
/**
 * 子を並べない親（`layout: free`）の下に `fill` を書いていないか（docs/03「Box」。Figma と同じく `fill` は
 * 子を並べる親の下でだけ意味を持つ）。
 *
 * スキーマの `enabledWhen` で閉じられないのは、条件が**親の** prop だから（docs/03「`enabledWhen` は単純な
 * 等値・不等値のみ」）。向きを持つかの判定は `Layout.direction` を引くので、コンパイル側の `fill` の出し分けと
 * 同じ答えになる。
 *
 * 親を引数で要求するので、親が決まらない位置（部品のルート）はそもそも呼ばれない。部品インスタンスの中身も
 * 対象外で、検証が見るのは**定義時点の props** だけ。
 *
