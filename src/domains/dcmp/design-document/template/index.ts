 ...
 *
 * デフォルトテーマと初期部品セットは片方だけでは成立しない（部品の見た目の prop はテーマのトークンを
 * 参照する）ため1つの型にまとめる。
 *
 * 部品が参照するトークンが揃っているかは型では縛らない。参照の整合はドキュメント全体の規則なので
 * `DesignDocument.collectErrors` が dangling-token として報告する担当で、docs/04-tokens.md も「通常の
 * バリデーションエラーとして検出される（特別扱いしない）」としている。
 */
