import { ExpandedNodeError } from "@/domains/dcmp/expanded-node";
import type { NodeTreeEditError } from "@/domains/dcmp/node-tree";
import { TokenEditError } from "@/domains/dcmp/token";

/**
 * ドキュメントの編集操作（挿入・削除・並べ替え・移動・部品化・解除・tokens 編集）が失敗する理由。
 * 呼び出し側が種類で分岐できるよう、メッセージ文字列ではなく直和で列挙する。
 *
 * `NodeTreeEditError` / `TokenEditError` / `ExpandedNodeError` をそのまま含む。いずれの失敗もドキュメントの
 * 失敗でもあるので、部分型として受け取れるよう同じ形のメンバを並べている（変換を挟まずに伝播できる）。
 */
