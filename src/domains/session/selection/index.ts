 ...
 * 今選ばれているものの正体。
 *
 * 名前と種別を 1 つの型にまとめるのは、この 2 つが**常に対で意味を持つ**ため（名前だけでは何を選んだのかが決まらず、
 * 種別だけではどれを選んだのかが決まらない）。表示側はこの対を受け取れば 1 度の判定で描ける。
 *
 * 種別が `none` になるのはスキーマに無い `type` のノード。不正なドキュメントでも描画は残る（docs/03-schema.md
 * 「不正ファイル時の挙動」）ので、種別が分からないことをそのまま出すために既定の種別へ寄せない。
 */
