...
/**
 * 外部変更で届いた内容を取り込んだ結果（docs/05-architecture.md「外部編集の検知」）。取り込めたときは
 * ドキュメントだけ、拒んだときはエラー一覧だけを持つ。
 *
 * 「ドキュメントもエラーも持つ」中間状態を作らないのは、不正なファイルでは表示を差し替えず最後に正常だった
 * 状態を保つため（docs/03-schema.md「不正ファイル時の挙動」）。
 *
 * `errors` を非空タプルで縛らないのは、`fromParsed` が受け取るエラー一覧が非空であることは型に出ておらず、
...
export const DocumentReload = {
   ...
   *
   * テキストの解釈そのものを持たないのは、読み方が外部フォーマットの知識で `libs/` の担当だから。ここが足すのは
   * スキーマ検証だけで、解釈に失敗していればその時点で返す（検証を走らせる相手がいない）。警告という中間区分は
   * 無い（docs/03-schema.md「バリデーション仕様」）ので、検出できたエラーは 1 本の一覧にまとめて返す。
   *
