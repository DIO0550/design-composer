 ...
 *
 * 自動保存も外部変更の監視も「どのパスの、どのドキュメントか」が揃って初めて決まる。片方だけでは何も書き出せず
 * 何も監視できないため、対を 1 つの型にして名前を付ける。
 *
 * **ここが運ぶドキュメントはスキーマ検証を通っていない**（docs/03-schema.md「不正ファイル時の挙動」の「開く
 * 時」）。取り込みの `DocumentReload.reloaded` が検証済みを運ぶのと非対称で、その違いは型に出ない。消費側は
 * `DocumentError.collectFrom` で自分で集める前提で書く。
 */
...
export const OpenedDocument = {
   ...
   *
   * **スキーマ検証はしない。** 不正でもそのまま開いてエラー一覧として画面に出す（docs/03-schema.md「不正ファイル時
   * の挙動」の「開く時」）。自動保存は画面の内容をそのまま書き出すのでアプリ内の編集で作った不正はファイルにも
   * 載り、ここで落とすと直す手段が外部エディタにしか無くなる（#158）。
   *
   * 取り込みと同じ `DocumentReload` へ委ねないのは、開いている最中の外部変更が「最後に正常だった状態を保つ」と
   * いう別の規定に従うため。`create(path, document)` を公開しないのは、「解釈できたならそのまま開く」という線引きが
