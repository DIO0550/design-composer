...
export const OpenedDocument = {
   ...
   *
   * **スキーマ検証はしない。** 不正でもそのまま開いてエラー一覧として画面に出す（docs/03-schema.md
   * 「不正ファイル時の挙動」の「開く時」）。自動保存は画面の内容をそのまま書き出すので
   * アプリ内の編集で作った不正はファイルにも載り、ここで落とすと直す手段が外部エディタ
   * にしか無くなる（#158）。
   *
   * 取り込みと同じ `DocumentReload` へ委ねないのは、開いている最中の外部変更が「最後に
   * 正常だった状態を保つ」という別の規定に従うため。`create(path, document)` を公開しな
   * いのは、「解釈できたならそのまま開く」という線引きが呼び出し側へ移り、開く経路が増
   * えるたびにそこで決め直すことになるため。
   *
   *   @param path このドキュメントの保存先
   *   @param parsed 読み込んだ中身を解釈した結果
   *   @returns 解釈できていれば保存先と対にしたドキュメント。解釈に失敗していれば  その
   *   理由（載せる相手のドキュメントが組み立っていないので開けない）
   */
  fromParsed(
    path: string,
