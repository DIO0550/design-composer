...
/**
 * 開いているファイルが、今画面に映っているドキュメントとして読めるか（docs/03-schema.md
 * 「不正ファイル時の挙動」）。不正なときだけ、拒んだ理由と「いつから食い違っているか」
 * を持つ。
 *
 * エラー一覧と起点を別々のフィールドで並べないのは、「エラーが空なのに起点がある」「エ
 * ラーがあるのに起点が無い」が表現できてしまうため。`errors` を非空タプルで縛らないのは
 * `DocumentReload` と同じ理由。
 *
 * 名前を隣の `DocumentSaveState` に揃えないのは、あちらが「保存という操作の状態」なのに
 * 対し、こちらは操作ではなく「ファイルが妥当かどうか」だから。
 */
...
export const FileValidity = {
   ...
   * 差し替わっていない以上、ファイルと食い違い始めた時刻も動かないため。拒むたびに採り
