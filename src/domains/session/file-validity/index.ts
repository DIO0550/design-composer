...
/**
 * 開いているファイルが、今画面に映っているドキュメントとして読めるか（docs/03-schema.md「不正ファイル時の
 * 挙動」）。不正なときだけ、拒んだ理由と「いつから食い違っているか」を持つ。
 *
 * エラー一覧と起点を別々のフィールドで並べないのは、「エラーが空なのに起点がある」「エラーがあるのに起点が
 * 無い」が表現できてしまうため。`errors` を非空タプルで縛らないのは `DocumentReload` と同じ理由。
 *
 * 名前を隣の `DocumentSaveState` に揃えないのは、あちらが「保存という操作の状態」なのに対し、こちらは操作では
 * なく「ファイルが妥当かどうか」だから。
 */
...
export const FileValidity = {
   ...
   * 取り込み結果を反映した妥当性。
   *
   * 不正が続いている間、起点は**最初に不正になった時刻のまま**にする。映っている描画が差し替わっていない以上、
   * ファイルと食い違い始めた時刻も動かないため。拒むたびに採り直すと、外部エディタで保存し直すたびに数字が 0 へ
   * 戻り、「いつから古いか」を答えなくなる。
   *
