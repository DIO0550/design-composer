 ...
 *
 * プリミティブは `type`、部品インスタンスは `ref` と書き出す先が異なるので、直和にして
 * 「両方持つ」「どちらも持たない」指定を表現できなくする。名前を持たないのは、一意な名
 * 前が挿入先のドキュメントを見ないと決まらないため（採番は `toNode` が行う）。
 *
 * カテゴリが `session` なのは、これがファイルに書かれる値ではなく**編集操作の指定**で
 * `.dcmp` に現れないため（import は `dcmp` だけなので機械では決まらない）。
 */
