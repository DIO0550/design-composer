 ...
 *
 * 掴んだ位置しか持たない状態と、落ちる先まで決まった状態を分けて列挙するのは、「動かして
 * いないのに落ちる先がある」を書けなくするため（`features/canvas` の `NodeDrag` と同じ形）。
 *
 * `dropped`（離した直後）を持たないのは、行をまたいで離すと行の `onClick` が発火しないため。
 * `click` は押した要素と離した要素の共通の祖先へ飛ぶので、群の器（`<ul>`）が受ける。
 *
