...
test("文字列を期待する位置に文字列があるとその値が取り出せる", () => {
  expect(Result.unwrap(Json.string("hello", "name"))).toBe("hello");
});

test("数値を期待する位置に数値があるとその値が取り出せる", () => {
  expect(Result.unwrap(Json.number(16, "width"))).toBe(16);
});

test("必須フィールドが存在するとその値がデコードされる", () => {
  const record = { name: "screen" };
...
test("任意フィールドが存在しないと未設定として扱われる", () => {
  expect(
    Result.unwrap(Json.optional({}, "", "props", Json.string)),
  ).toBeUndefined();
});
...
test("配列は要素ごとにデコードされて順序が保たれる", () => {
  const decoded = Json.arrayOf(["a", "b"], "names", Json.string);
