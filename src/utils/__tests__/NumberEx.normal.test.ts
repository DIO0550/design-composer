...
test("指定した桁で丸めると二進小数の端数が落ちる", () => {
  expect(NumberEx.round(0.7000000000000001, { decimals: 4 })).toBe(0.7);
});

test("指定した桁より下は四捨五入される", () => {
  expect(NumberEx.round(12.3456789, { decimals: 4 })).toBe(12.3457);
});

test("小数第1位まで丸める", () => {
  expect(NumberEx.round(39.2156862745098, { decimals: 1 })).toBe(39.2);
});

test("負の半端値は0に近い側へ丸まる", () => {
  // 半端値が +∞ の側へ寄るのは `round` 自身の仕様（@returns に書いてある）。
  // 負の比率を綴る経路がここを通るので、寄る向きを固定する
  expect(NumberEx.round(-0.15, { decimals: 1 })).toBe(-0.1);
});
