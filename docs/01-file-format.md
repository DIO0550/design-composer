...
major・minor はそれぞれ 0 以上の整数を 10 進で書き、`0` 以外の数の先頭に `0` を付けない（`"01.2"` は不正。1 つの版の綴りを 1 通りにするため）。JavaScript の `Number` で正確に表せない大きな数（`Number.MAX_SAFE_INTEGER` を超えるもの）も不正とする。
...
- 対象: 部品名、artboard 名、すべてのノードの `name`、トークン名（04-tokens「命名規則」）
- kebab-case、使用可能文字は `[a-z0-9-]`
- `/` `#` `.` は将来のパス修飾（ファイル跨ぎ参照）のために予約し、使用を禁止する
- **数字だけの名前（`3` / `20` / `007`）は禁止する。** 部品名とトークン名はオブジェクトのキーに書かれ、読み込んだオブジェクトでは配列の添字として読めるキー（`3` / `20`）が書いた位置に関わらず先頭へ数値順に並ぶため、定義順が保てない
  - 並び替わらない `007` も禁じるのは、規則を「数字だけ」の 1 文で説明できる形に揃えるため
  - 配列に並ぶ artboard 名・ノード名も同じ規則に従う（識別子の規則は 1 つに保つ）
