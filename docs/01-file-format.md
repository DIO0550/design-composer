...
**アプリが書き出す現行版は `"1.3"`。** prop や artboard のフィールドを足すたびに minor を上げる（上げないと、新しい綴りを含むファイルが旧アプリで「読めるが未知のフィールド」として開かれ、どこまで解釈されたかが利用者から見えない）。
...
| 1.3 | ノードの `constraintX` / `constraintY`（03-schema「親のリサイズへの追従」） |
