...
| `x` | number | - | キャンバス上の位置。**枠の左上**の X 座標（px）。負も可。`y` と対で書く |
| `y` | number | - | キャンバス上の位置。**枠の左上**の Y 座標（px）。負も可。`x` と対で書く |
