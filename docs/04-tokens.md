...
- **`background` は colors と gradients のどちらも指せる**（03-schema「塗り」）。2 種別にまたがる prop はこれだけ
- **種別のキーは省略してよい**（省略は空として読む）。グラデーションを持たないドキュメントに `gradients` を書かせないため。省いた種別を参照している prop は dangling 参照になり、通常のバリデーションエラーとして出る（下記「スキーマデフォルトとの関係」）
...
"tokens": {
  ...
  "gradients": { "brand-fade": { "angle": 180, "stops": [{ "color": "#3b82f6", "position": 0 }, { "color": "#1d4ed8", "position": 1 }] } }
}
```
...
- 一意性は種別内で保証する（colors と spacing に同名があってもよい。参照時に引く種別がスキーマで 1 つに決まるため衝突しない）
- **例外: 1 つの prop が複数の種別を指せる場合、その種別の間では同名を許さない**（`"background": "brand"` がどちらの種別か決まらなくなる）。重複はバリデーションエラー（03-schema「バリデーション仕様」）。今あたるのは塗りの colors と gradients だけ
...
| フィールド | 型 | 必須 | 内容 |
|---|---|---|---|
| `angle` | number | ✔ | 度。CSS の `linear-gradient(<angle>)` と同じ基準（`0` が下から上、`180` が上から下） |
| `stops` | 配列 | ✔ | 色の変わり目。**2 件以上**、`position` の非減順（同じ位置が並んでもよい） |

`stops` の各要素のフィールド:

| フィールド | 型 | 必須 | 内容 |
|---|---|---|---|
| `color` | string | ✔ | **生 hex**（形式は上記「colors」と同じ `#rrggbb` / `#rrggbbaa`） |
| `position` | number | ✔ | 単位なしの 0〜1。`0` が始まり、`1` が終わり |

- **トークンの値に並び（配列）が現れるのはこの種別だけ。** `props` の値をスカラーに限る規定（02-data-model「値の形: フラットなスカラーのみ」）は props の話なので、トークンの値には及ばない
- `stops` の `color` は colors トークンへの参照ではなく生 hex で持つ。影の色と同じ判断（上記「shadows」）で、グラデーション用の中間色を colors パレットへ混ぜない
- **同じ `position` の stop が並ぶのは許す。** CSS で色が段で切り替わる書き方として意味が決まる
- **グラデーションの種類を表すフィールドは持たない。** 初期仕様は linear だけで、値が 1 つしか無い欄を先回りで置かない。radial / conic を足すときに判別子を足し、省略時を linear と決めれば既存のファイルはそのまま読める（01-file-format「formatVersion」の minor の追加的変更）
- **`angle` に値域は宣言しない。** 1 周を超える角度も「1 周と◯度」として意味が決まる（03-schema「回転」と同じ）
- **値域と並びのうちエラーにするのは `stops` の件数だけ**（2 件未満。03-schema「バリデーション仕様」）。`position` が 0〜1 の外にあること・非減順になっていないことはエラーにしない。下記「値域の扱い」のとおり値域は編集で課すもので、範囲や並びが崩れても参照の解決はでき、描画も成立する（必須フィールドの欠落・型違いは値域の話ではなく、構造がスキーマの形に合わない側 / 03-schema「不正ファイル時の挙動」）
- アプリが読み書きできるようになってから 01-file-format「formatVersion」の表へ足す（まだ足していない）

### 値域の扱い

- 値域は**編集で受け取るところ**で課す。範囲外の入力は値を変えず、画面には何も出さない（数値として読めない入力と同じ扱い）
- **件数の不足だけは値域と別に扱う。** グラデーションの `stops` が 2 件未満だと描くものが決まらないので、そちらはバリデーションエラーにする（上記「gradients」）
...
- **2 種別を指せる prop（`background`）の既定も、名前だけで書く**（解決が一意になる理由は上記「命名規則」の例外）
...
- **叩き台は `gradients` のキーを持たない**（省略 = 空。上記「種別」）。初期部品セット（下記）にグラデーションを使う部品が無く、塗りは colors だけで足りるため。使うときはトークンパネルから足す
