...
- **`background` は colors と gradients のどちらも指せる**（03-schema「塗り」）。塗りを参照する prop が 2 種別にまたがるのはこれだけ
- **種別のキーは省略してよい**（省略は空として読む）。グラデーションを持たないドキュメントに `gradients` を書かせない
...
"tokens": {
  ...
  "gradients": { "brand-fade": { "angle": 180, "stops": [
    { "color": "#3b82f6", "position": 0 }, { "color": "#1d4ed8", "position": 1 }
  ] } }
}
```
...
- **例外: 塗りが参照する colors と gradients の間では同名を許さない**（`"background": "brand"` がどちらの種別か決まらなくなる）。重複はバリデーションエラー（03-schema「バリデーション仕様」）
...
複合オブジェクト。フィールドは以下に固定する。**トークンの値に並び（配列）が現れるのはこの種別だけ**で、`props` の値をスカラーに限る規定（02-data-model「値の形: フラットなスカラーのみ」）は props の話なのでここには及ばない。

| フィールド | 型 | 必須 | 内容 |
|---|---|---|---|
| `angle` | number | ✔ | 度。CSS の `linear-gradient(<angle>)` と同じ基準（`0` が下から上、`180` が上から下） |
| `stops` | 配列 | ✔ | 色の変わり目。**2 件以上**、`position` の昇順 |
...
- `stops` の `color` は colors トークンへの参照ではなく生 hex で持つ。影の色と同じ判断（上記「shadows」）で、階調用の中間色を colors パレットへ混ぜない
...
- **`stops` が 2 件未満のときだけバリデーションエラーにする**（03-schema「バリデーション仕様」）。`position` が 0〜1 の外にあることはエラーにしない — 下記「値域の扱い」のとおり値域は編集で課すもので、範囲を外れても参照の解決と描画は成立する
- **どの版から読み書きできるかは 01-file-format「formatVersion」の表が持つ。** 種別の追加は minor の追加的変更にあたるが、アプリがグラデーションを読み書きできるようになった時点で表へ足す（まだ足していない）
...
- **2 種別を指せる prop（`background`）の既定も、名前だけで書く。** 塗りの 2 種別の間で同名を許さない（上記「命名規則」）ので、どちらの種別に解決されるかは一意に決まる
...
- **`gradients` だけは空で同梱する。** 初期部品セット（下記）に階調を使う部品が無く、空の種別を持てる（上記「種別」）ため。塗りは colors だけで足りる
