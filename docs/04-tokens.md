...
- **CSS へは `linear-gradient({angle}deg, {color} {ratio*100}%, ...)` で出す。** `ratio` は 100 倍して `%` にする。**ここで落とすのは二進小数の端数だけで、値域は動かさない**（範囲外の `ratio` も `stops` の件数も上記のとおりそのまま出す）。100 倍した時点で `0.007` が `0.7000000000000001` になるので、小数第4位より下は四捨五入する
