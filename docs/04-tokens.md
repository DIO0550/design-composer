...
- **CSS へは `linear-gradient({angle}deg, {color} {ratio*100}%, ...)` で出す。** `ratio` は 100 倍して `%` にし、小数第4位より下は四捨五入する（`0.007` を `0.7000000000000001%` と綴らないため。二進小数で表せない比率は 100 倍した時点で端数が出る）
