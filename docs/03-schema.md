...
Box と Ellipse が面の塗りを `background`、Text が文字の塗りを `color` で持つ。**この節が決めるのは塗りを何件・どの形で持てるかで**、prop の既定は各プリミティブの節（下記「Box」「Text」「Ellipse」）が持つ。

- **塗りは高々 1 件。グラデーション・複数フィルは持たない。** Figma は複数のフィルを重ねられ、各フィルが単色 / グラデーション / 画像を取るが、こちらは colors トークンの参照 1 件に限る。理由は 3 つ:
  - **グラデーションを colors の値にすると、トークン 1 件の値のフィールドが固定でなくなる。** 広色域への拡張（04-tokens「値の形式」）は色 1 つの表記を変えるだけだが、グラデーションは**色と位置の並び**なので、値の中に件数が入る。複合値の shadows / typography はどちらもフィールドが固定で、可変長の並びを値に持つ種別は無い
  - **6 つ目のトークン種別を足すかは、この prop だけの判断にできない**（02-data-model「値のドメイン: 3種類」。`opacity` を生リテラルにしたのと同じ理由）。種別を増やす判断が要る他の見た目の値と合わせて 1 回で決める
  - **`background2` のように prop を並べる形にはしない。** padding の 4 辺・角丸の 4 隅は数が形から決まるが、フィルの枚数は決まらない。**1 つのノードの中でフィルを重ねる順序**とフィルごとのブレンドモードも持たないので、並べても重なりの見た目が決まらない（同一親の子の重ね順は持つ / 06-ui「編集操作の一覧」）
- **半透明の Box を重ねても代用にはならない。** 薄い Box に `rotation` を書けば線になる（上記「プリミティブの初期セット」）のと違い、重ねて作れるのは色の切り替わりで、連続した色の変化にはならない
- **持たないのは初期仕様としてで、将来拡張として予約する。** 読み直すのは、種別を増やす判断をまとめてするとき / 1 つのノードの中でフィルを重ねる順序とブレンドモードを持つと決めたとき / 広色域と同じく formatVersion を上げるとき。UI 案 `docs/Design Composer.html` が塗りに描いているのも `background` の 1 行（見本とトークン名を収めたプルダウン 1 つ）で、この決定と矛盾しない
- Image を導入するときに、画像を塗りとしてどう持つかはそのときに決める（上記「プリミティブの初期セット」の「Image は保留」）
...
| `background` | トークン (colors) | 上記「塗り」 | なし (透明) |
...
| `color` | トークン (colors) | 上記「塗り」 | デフォルトトークン |
...
| `background` | トークン (colors) | 上記「塗り」 | `gray-300` |
| `shadow` | トークン (shadows) | | なし |
| `opacity` | | 上記「不透明度」 | |
| `visibility` | | 上記「表示 / 非表示」 | |
