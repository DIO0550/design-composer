...
- **hex でない色トークン**（colors の値が `#rrggbb` / `#rrggbbaa` の正規形でない。影・グラデーションの中の色は見ない / 04-tokens「colors」）
...
| 上記を通ったうえでのバリデーション違反（未知の `type` / 未知の prop / ドメイン違反 / dangling ref / 循環参照 / 未宣言の overrides / binding の不整合 / 識別子規則違反 / `name` 欠落 / 名前の一意性違反 / 塗り用の 2 種別での名前衝突 / hex でない色トークン） | **開く**。エラー一覧を重ねて表示し、編集は続けられる |
