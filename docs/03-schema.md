...
Box と Ellipse が面の塗りを `background`、Text が文字の塗りを `color` で持つ。**この節が決めるのは塗りを何件・どの形で持てるかで**、prop の既定は各プリミティブの節（下記「Box」「Ellipse」「Text」）が持つ。

- **塗りは高々1件。グラデーション・複数フィルは持たない。** Figma は複数のフィルを重ねられ、各フィルが単色 / グラデーション / 画像を取るが、こちらは colors トークンの参照1件に限る（Box は既定では塗りを持たない）。理由は3つ:
  - **グラデーションを colors の値にすると、トークン1件の値が固定長でなくなる。** 広色域への拡張（04-tokens「値の形式」）は色1つの表記を変えるだけだが、グラデーションは**色と位置の並び**なので件数が値の中に入る。複合値の shadows / typography はどちらもフィールドが固定で、可変長の並びを値に持つ種別は無い
  - **6つ目のトークン種別を足すかは、この prop だけの判断にできない**（02-data-model「値のドメイン: 3種類」。`opacity` を生リテラルにしたのと同じ理由）。同じ制約に当たるぼかし（layer blur / background blur）と合わせて1回で決める
  - **`background2` のように prop を並べる形にはしない。** padding の4辺・角丸の4隅は数が形から決まるが、フィルの枚数は決まらない。重ね順とブレンドモードを持たないので、並べても重なりの見た目が決まらない
- **半透明の Box を重ねて代用する形も採らない。** 薄い Box に `rotation` を書けば線になる（上記「プリミティブの初期セット」）のと違い、重ねて作れるのは色の切り替わりで、連続した色の変化にはならない
- **持たないのは初期仕様としてで、将来拡張として予約する。** 読み直すのは、ぼかしと合わせて種別を増やすかを判断するとき / 重ね順とブレンドモードを持つと決めたとき / UI 案が塗りの欄を変えたとき。UI 案（`docs/Design Composer.html`）は塗りを**見本 + トークン名 + プルダウン1つの1行**として描いており、フィルを足す入口もグラデーションの編集欄も描いていない
- 画像塗りは Image プリミティブに従属するので、ここでは決めない（上記「プリミティブの初期セット」の「Image は保留」）
- artboard は Box スキーマを流用するので**`background` を受け付ける**（01-file-format「artboards」が `"props": { "background": "primary" }` を例示している）。持てる件数もノードと同じ
