...
- **理由: 経緯はそのセッションの中にしか無い。** どこまで読んだか・何を確かめて選択肢を
  ...
  **別のセッション**が辿れなくなる(`harness-record` の「推測で埋めない」が効かなくなるのも
  同じ理由)
- **止まってから残すのでは遅い。** #135 は調査・分割・判断確定の 3 コメントすべてに URL が
  無く、**調査を書いたセッションはあとから特定できなかった**(トリガー起動のセッションは
  `list_sessions` の一覧にも載らない)。止まるかどうかは着手時には決まっていないので、
  起点を「止まったとき」ではなく「着手したとき」に置く
- 通知(Discord 等)にだけ載せるのでは足りない。**通知は流れるが Issue は残る**
- **層 3 まで上げてある。** `session-url-notice.sh`(SessionStart)がセッションの URL を
  組み立てて渡す。**URL はセッションの中からしか作れない**ので、規約だけに置くと
  「残そうと思ったが URL が分からない」で止まる
- **Why not: 層 2・層 1 へは上げられない。** 見たいのは GitHub のコメントで、**リポジトリに
  痕跡が残らない**ため git hooks(層 2)も CI(層 1)も検査できない
  (`.claude/hooks/README.md`「カバー範囲と残る穴」の `block-npx.sh` と同じ型)
- **Why not: フックから直接コメントを投げない。** 宛先の Issue 番号がフックからは確実に
  決められない(ブランチが `claude/issue-<N>-...` の形とは限らない)。間違えたコメントは
  消しても通知が残る
