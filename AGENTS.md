...
**PR 本文に `Closes #<Issue番号>` を書く。** 閉じる側は GitHub のマージに担わせ、
書き忘れは CI が落とす(`.github/workflows/pr-closing-keyword.yml`)。手で閉じる運用に
すると閉じ忘れが必ず出る。

- 着手した Issue は、その回の PR のマージで閉じる。持ち越さない
- **スコープ外が残った・決定事項が変わったときは、その Issue を開け直さず新しい Issue を
  立てる**(元の Issue からリンクする)。分割して別の Issue にした分は、その Issue に
  着手した回で閉じる(分割元は自分の分が終わるまで開いたまま → 「タスクの分割」)
- Issue を持たない PR(記録・棚卸しなど)は、本文に `Issue 無し: <理由>` の 1 行を書く。
  **書き忘れでは通らない形にするための申告**なので、理由が空なら CI は落ちる

**理由: 閉じ忘れが混ざると、assign 付き open が進行中の目印として使えなくなる。**
「開いている」がこれからやるのか・途中で止まったのか・決着したが閉じ忘れたのかを、
一覧からは区別できない。
