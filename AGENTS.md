...
**PR 本文に `Closes #<Issue番号>` を書く。** 閉じる側は GitHub のマージに担わせ、書き忘れは
CI が落とす。着手した Issue は持ち越さず、スコープ外が残った・決定事項が変わったときは、
その Issue を開け直さず**新しい Issue を立てる**(元の Issue からリンクする)。分割元は自分の
分が終わるまで開いたままにし、自分の分が残らないなら最後の子を閉じる回に一緒に閉じる。

**理由: 閉じ忘れが混ざると、assign 付き open が進行中の目印として使えなくなる。**
「開いている」がこれからやるのか・止まったのか・決着したが閉じ忘れたのかを区別できない。

手順(Issue を持たない PR の申告・マージ後に閉じたことの確認)は `implementation-flow`
フェーズ 7・8。
