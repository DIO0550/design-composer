...
**着手したら自分をアサインし、その回の作業(PR のマージ)で閉じる。** 続きが要るものは
Issue を開けたままにせず、**新しい Issue を立てて元の Issue からリンクする**。

- 閉じる起点は **PR のマージ**。**PR を出す Issue は**本文に `Closes #<番号>` を書き、
  手では閉じない(検査は `.github/workflows/pr-closing-issue.yml`。
  例外は `harness-record` が出す記録 PR だけ)
- **その回に変わった判断は、閉じた Issue へコメントで残す**(`implementation-flow` フェーズ 8)。
  新しい Issue にするのは**続きの作業が要るものだけ**で、閉じた Issue は開け直さない
- 着手したが PR を出さずに止まったものと、子へ分割した親 Issue は open のままでよい。
  どちらも PR を持たないので閉じる起点が来ない(親は子が全部閉じた時点で手で閉じる)

**理由: open な Issue を「まだ終わっていないもの」だけに保つため。** 閉じ忘れが混ざると、
open が「これからやる」「途中で止まった」「もう決着した」のどれなのかを一覧から区別できず、
アサインを進行中の目印として使えなくなる。
