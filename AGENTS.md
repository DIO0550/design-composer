...
### 判断待ちで止まるときは、セッションの URL を Issue に残す

回答を待って作業を止めるときは、**選択肢と根拠を書いた Issue のコメントに、作業中の
Claude Code セッションの URL を併記する**(`https://claude.ai/code/session_<id>`)。

- **理由: 止まった時点の経緯は、そのセッションの中にしか無い。** どこまで読んだか・
  何を確かめて選択肢を絞ったかは Issue のコメントに書ききれず、Issue だけを見ている人と、
