...
- 判例を書けるのは `harness-growth` だけ。**機械判定できない再発の介入先は判例**で、フックと
  合わせた 2 択の片方になっている(`harness/case-law/README.md`)

**理由: 判例は毎回増え、規範は増えない。** 同じファイルに置くと常時ロードが単調増加する。

## 常時ロードはラチェットで縮める

**`AGENTS.md` + `rules/` の合計行数には予算があり、予算は下げるだけで上げない。** 足したら
同量を削る。削り先が出せないなら、その追加は `rules/` ではなく判例・フックのどちらかに置く。
実測が予算を下回ったら、その回に予算を実測まで下げる。

- 検査は `bash harness/records/count.sh --ratchet`(ずれていれば exit 1)。予算は同ファイルの
  `LoadedBudget` が持ち、git hooks と CI が走らせる
- **固定上限にしない。** 上限方式は「超えるまで増え続ける」ことを許すので、超えた時点で必ず
  大きな棚卸しが要る
...
マージ後は `harness-record` スキルでその回の評価を記録する。記録を数えてフック・判例へ手を
入れるのは `harness-growth` スキルで、別の機会に行う。
...
レビューで新しい判断基準が示されたら、その場の修正で終わらせず反映する。**人・bot・CI へ
届いた指摘が2回以上出たら、規範の抜けとして扱う。**

回数を数える材料は `harness/records/` に溜まる(マージのたびに `harness-record` スキルが記録を
1ファイル追加する)。数えるのは `harness/records/count.sh`で、手順は `harness-growth` スキル。

- **通算ではなく「最後の介入以降の再発数」で見る**(通算は単調増加するので効いたかを表さない)。
  サブエージェントが捕まえた指摘と自己修正は別の列(`内部`)にして**分岐には使わない**
  (理由は `harness-record/templates/record.md`「なぜ外部だけで分岐するのか」)
- **介入先はフックか判例の2択で、1回につき1つ。** `rules/` は増やさない。規範そのものが
  足りないと判断したときは、その場で書き換えず**規範の追加を提案する Issue を立てる**

**規約に足すときは、同じファイルが既に持っている記述・例と突き合わせる**(判定文が規約自身の
例で逆の答えを出した / 前の節と矛盾した / 表の行が網羅していなかった、が実際に起きている)。
...
`session-url-notice.sh`(SessionStart)が促すところまでで、**機械では強制できない**
(理由は [`harness/case-law/process.md`](harness/case-law/process.md))。
