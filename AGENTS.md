...
**`AGENTS.md` + `rules/` の合計は、`harness/records/count.sh` の `always_loaded_cap` を超えない。**
足すなら同量を削る。上限は縮めたときに下げるだけで、上げない。

削ることは足すことと同じ重さの介入で、後回しにしない。常時ロードの 1 行は書く側が 1 回払うの
ではなく読む側が毎セッション払い、積もった文章は初期の設計を固定して次の回が試す範囲を狭める。
上限を固定の閾値ではなくラチェットにしている理由は [`harness/case-law/process.md`](harness/case-law/process.md)。
...
数えるのは `harness/records/count.sh` で、**人・bot・CI へ届いた指摘が最後の介入より後に
2 回以上**ある分類にだけ手を入れる(手順は `harness-growth`)。
