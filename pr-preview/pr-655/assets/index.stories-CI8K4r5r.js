...
}`,...E.parameters?.docs?.source},description:{story:`ファイルとの同期に失敗している状態。実体の無いパスは監視を張れないので、失敗の帯が編集画
面の上に出る。表示そのものは保たれることをここで確認できる。`,...E.parameters?.docs?.description}}},A.parameters={...A.parameters,docs:{...A.parameters?.docs,source:{originalSource:`{
...
}`,...A.parameters?.docs?.source},description:{story:`ドキュメント自身が不正な状態。アプリ内の編集で使用中トークンを消したあとも、その内容が自
動保存されたファイルを開き直した直後も、画面はこれになる。
...
}`,...M.parameters?.docs?.source},description:{story:`絶対配置のノードが最初から居る編集画面。
...
絶対配置を持ち込むとその意味が変わる。`,...M.parameters?.docs?.description}}},F.parameters={...F.parameters,docs:{...F.parameters?.docs,source:{originalSource:`{
