...
- `.oxlintrc.json` の `overrides`(`rules/architecture.md`「モジュールの公開API」の「複数ファイルへの分割が必要になったら」の閾値。`分類: module-api`、#600)は、`src/**/*.tsx` に `max-lines: 600` を `error` で置く。導入時点の違反は 0 件(非テストの `.tsx` の最大は `opened-document-editor/index.tsx` の 575 行)。捕まえたいのは pr-240 の `artboard-canvas/index.tsx`(716 行・11 コンポーネント)の形で、**閾値はその 716 と現行最大 575 の間に置いている**。余裕が 25 行しかないのは意図で、倍に育ってから気づく形をやめるのが目的
  - `src/**/__tests__/**/*.tsx` は後勝ちの override で `off`。`src/**/*.tsx` 全体へ当てると `use-editor-state.actions.test.tsx`(620 行)が 1 件落ちるが、**テストの肥大化は `rules/testing.md`「1ファイルが肥大化したらカテゴリを分けること」が別の軸で持っており、`module-api` として数えた再発でもない**。このテストをカテゴリで割って 0 件にすれば、除外そのものを外せる(その時点でこの限定は不要になる)。テストだけ緩い閾値(800 等)を置かないのは、根拠になる件数がどこにも無く、数字を発明することになるため
  - **`.stories.tsx` は対象に含める。** ストーリーはモジュールごと(`index.stories.tsx`)に置くので、肥大化は `.tsx` 本体と同じ「部品が積み上がる」サイン。現行最大は 338 行
