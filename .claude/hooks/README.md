...
- `.oxlintrc.json` の `overrides`(`rules/architecture.md`「モジュールの公開API」の「複数ファイルへの分割が必要になったら」の閾値。`分類: module-api`、#600)は、`src/**/*.tsx` に `max-lines: 600` を `error` で置く。導入時点の違反は 0 件(非テストの `.tsx` の最大は `opened-document-editor/index.tsx` の 575 行)。捕まえたいのは pr-240 の `artboard-canvas/index.tsx`(716 行・11 コンポーネント)の形で、**閾値はその 716 と現行最大 575 の間に置いている**。余裕が 25 行しかないのは意図で、倍に育ってから気づく形をやめるのが目的
  - `src/**/__tests__/**/*.tsx` は後勝ちの override で `off`。`src/**/*.tsx` 全体へ当てると `use-editor-state.actions.test.tsx`(620 行)が 1 件落ちるが、**テストの肥大化は `rules/testing.md`「1ファイルが肥大化したらカテゴリを分けること」が別の軸で持っており、`module-api` として数えた再発でもない**。このテストをカテゴリで割って 0 件にすれば、除外そのものを外せる(その時点でこの限定は不要になる)。**埋めるのが 1 ファイルだけなのにこの回で埋めなかった**のは、`src/` のテストを割る差分がハーネスの差分に混ざると、どちらの判断も十分に確かめられないため(同じ回に Step 3 の棚卸し 6 件を見送ったのと同じ理由)。テストだけ緩い閾値(800 等)を置かないのは、根拠になる件数がどこにも無く、数字を発明することになるため
  - **`.stories.tsx` と `__stories__/` 配下の `.tsx` は対象に含める。** ストーリーはモジュールごと(`index.stories.tsx`)に置くので、肥大化は `.tsx` 本体と同じ「部品が積み上がる」サイン。現行最大は `opened-document-editor/index.stories.tsx` の 338 行で、共有の器(`artboard-canvas/__stories__/canvas-controls.tsx` 等)は 69 行
  - **`.ts` は対象外。** `design-document/index.ts` が 1449 行、`editor-state/index.ts` が 1273 行あり、違反 0 件では入れられない。そのため**中身を兄弟の `.ts` へ逃がす経路は開いたまま**で、止めているのは `rules/architecture.md`「実装は `index.ts` に直接書く」という観点だけ(`import-rule-violations.py` が見るのは「外から非 index ファイルを読む」形で、`index.tsx` からだけ読む兄弟 `.ts` は検出しない)
  ...
  - **この節を書いた時点では、これは事実ではなかった。** `lib/lint-suppressions.py` の検出は `biome-ignore` / `eslint-disable` の 2 綴りしか見ておらず、oxlint が同じく読む `oxlint-disable` の綴りは素通りしていた(実測: `// oxlint-disable eslint/max-lines` の 1 行で、`@lint-suppress-ok` 無しにルールを無効化できた)。同じ差分で検出側へ `oxlint-disable` を足して塞いである。既存の抑制は `biome-ignore` の 3 件だけなので、追加による新しい違反は 0 件
