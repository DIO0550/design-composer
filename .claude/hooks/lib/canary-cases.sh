...
# フックが素通りしても気づけないからで、この表は落ちれば分かる。python3 が無い環境では
# deny 側が全件、jq が無い環境では下の注意のとおり一部が NG になるので、判定が変わったと
# 読み違えないこと(そのため `harness/githooks/pre-push` は両方が揃う環境でだけ走らせる)。
set -uo pipefail
