...
**同じ Issue に別のセッションが既に着手していないかを、URL をコメントする前と後の 2 回読む。**
CI の `duplicate-issue-pr`(`.github/workflows/pr-closing-issue.yml`)が気づくのは PR が
開いた後で、そこまでの実装は片方が捨てることになる。

1. **コメントする前に、その Issue のコメントを読む。** 他のセッションの URL コメントがあり、
   その後にそのセッションの停止・判断待ち・引き継ぎの表明が無ければ、進行中とみなして止まる
2. 進行中の相手が無ければ、自分の URL をコメントする(`AGENTS.md`)
3. **コメントした後に、もう一度読む。** 自分より前に進行中の URL コメントがあれば、
   **後から来た側(自分)が止まる**。ほぼ同時に着手すると、1 回目ではどちらも相手が見えない
4. 止まるときは、並行している旨と選択肢(相手に任せる / 役割を分ける)を Issue にコメントする

- 起動の依頼が再開・引き継ぎを明示しているなら、止まっている相手の URL があっても進めてよい
- assignee は見ない。どのセッションも同じアカウントで動くので、自分と相手を区別できない
- `Closes #<番号>` を持つ open PR の一覧も手掛かりにしない。作成直後の PR が一覧に出ないことがある
