...
3. **想定した接続先・前提・検証手段が実例で本当に成立する/機能するかを確認する。**
   ...
   **検証コマンド自体が意図した範囲を覆っているか**(ディレクトリの深さ・拡張子の指定漏れ・
   ...
   フェーズでも呼ぶ。`plan-reviewer` は毎回捕まえていたのに計画の書き直しが繰り返されたため、
   渡す前に確かめる位置へ前倒しした)
...
7. **触るモジュール・呼び出し元の doc / Why コメントが「常に◯◯である」「〜のときのみ」と
   ...
   `plan-reviewer` は毎回捕まえていたのに計画の書き直しが繰り返されたため、レビューへ渡す前に
   確かめる位置へ前倒しした
8. **ドメインオブジェクトへロジックを帰属させる理由を計画に書いたら、その理由が対象の型に
   ...
   書き終えるたびに `claim-verification` スキルで確かめる**(`plan-reviewer` でも捕まらず
   人のレビューまで届いた形があり、渡す前に確かめる位置へ前倒しした)
...
bash harness/records/count.sh --ratchet                         # 常時ロードの行数（ラチェット）
```

- **下の 7 つは `pnpm` のスクリプトに無い。** `rules-check` の 5 つは
  git hooks と CI(`frontend.yml` の `rules-check`)が、`check-added-*` の 2 つは git hooks と
  CI の `lint-suppress` ジョブだけが走らせるので、この 7 行を省くと手元の確認がゲートより
  狭くなる。**doc コメントとテスト規約が CI へ上げられたのは、git hooks と Claude Code の
  フックが同じ環境で同時に抜けたため**(`.claude/hooks/README.md`「カバー範囲と残る穴」)
- **`check-added-*` の 2 つは base との差分で判定する。** 引数を省くと `origin/main` と
  比べるので、手元では引数なしで走る。CI だけが走らせていて push 前の一覧に無かった間に、
  CI で落ちて初めて気づく形が繰り返し出ている
- **カナリアが通っても、それだけでは不発と決まらない。** カナリアは自分の取りこぼしと本当の
  不発を区別できない。`.claude/hooks/README.md`「発火しているかを確かめる(カナリア)」の表で
  PreToolUse の痕跡を見て切り分ける。「不発」と記録してよいのは痕跡が無い枝だけ
...
- **不発と分かったら、このセッションで禁止コマンドを既に実行していないかも見直す**
  ...
  git hooks・CI に移せず(`.claude/hooks/README.md`「カバー範囲と残る穴」)、
  不発環境ではここでしか止まらない。Bash の実行履歴を見直し、該当があれば取り消す
  (`git add -A` を打ってしまったら `git reset` して個別に add し直す)
- **`&&` で 1 本に連ねない。** 連ねると途中の出力が流れて末尾しか見えず、「全部通った」と
  読み違える(実際に、format 差分のまま push して CI を落としている)。**カナリアを
...
3. **`harness-record` スキルでその回の評価を記録する**。記録を残すところまでで、集計と介入の
   判断は行わない(→ `harness-growth`)
