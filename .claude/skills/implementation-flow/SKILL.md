...
2. **一括置換・複数箇所の書き換えを伴う計画は、立てた収集条件を実際にコードへ当てて件数を
   ...
   件数(呼び出し側の依存・書き換えが要る既存テストの数を含む)が一致するかを確認する。
   **想定した接続先(既存ヘルパー・retarget 先の fixture 等)が実例で本当に成立するか**、
   **検証コマンド自体が意図した範囲を覆っているか**(ディレクトリの深さ・拡張子の指定漏れで
   一部しか見ていない、を含む)も同じ手順で確かめる(`分類: plan-scope-verification`)
3. **テストケースが前提とするヘルパー・フィクスチャ・API が実在し、想定通り動くかを確認する。**
   既存のヘルパーを流用する計画なら、そのヘルパーが確かめたい経路へ実際に到達できるかを
   コードを読んで確認する(到達できないなら、その場で計画を直す)
4. **ファイル表に挙げた各ファイルについて、`find-comment-references.sh` を実行する。**

   ```bash
   .claude/skills/implementation-flow/scripts/find-comment-references.sh <ファイル表のパス>...
   ```

   コードの `import` だけを grep したファイル表は、doc / Why コメントの中だけで対象を
   名指ししている参照を見落とす。出力される行は「ファイル表に入れるべきかを確かめる候補」
   であり、コメントの主張がこの差分で実際に嘘になるかは読んで判断する。嘘になるならその
   ファイルをファイル表へ追加する(`分類: plan-comment-reference`)
