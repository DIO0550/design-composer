...
8. **「未決の判断」欄は必ず書き、「無し」と締める前に、実装中に手が止まる判断を列挙して潰す。**
   計画本文・却下案・ゴール文を読み直し、「どちらでも実装できる」「あとで決める」「仕様(`docs/`)に
   根拠が無い」点を挙げる。表示の文言・0 件のときの見せ方・並び順・語の意味・モジュールを分けるか・
   スコープの内か外か・`docs/` へ何を書くかは、いずれも**実装中に手が止まる**ので着手前に決める。
   決めたものは計画へ、決めずに残すものは「未決の判断」へ書く(欄ごと省かない)。
   `plan-reviewer` は**この形を毎回捕まえている**(すり抜け 0)のに計画を書く側が繰り返し、
   計画の書き直しが続いたため、レビューへ渡す前に潰す位置へ前倒しした
   (`分類: plan-open-decision-unverified`)
