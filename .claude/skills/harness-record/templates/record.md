...
| `plan-comment-reference` | implementation-flow フェーズ 3(SKILL.md「計画」フェーズ 3 手順 4・`find-comment-references.sh`)(移動・書き換えの対象を doc / Why コメント内でだけ名指ししているシンボルが、コードの参照だけを拾ったファイル表に挙がっていない形) |
...
`comment-stale-edit` は pr-219 時点で `comment-mismatch` から分割された先だが、その
`comment-stale-edit` 自体が pr-219〜pr-305 の間に `comment-referent-drift` /
`comment-enumeration-drift` / `comment-premise-drift` / `comment-block-placement` へ
さらに分割した(観点層まで介入済み(pr-219)なのに再発 25 件で飽和 — 「2c. 介入後 5 回以上」)。
25 件の内訳は、名指ししたシンボル名・型名・引数名・パスが同じ差分内の改名・型変更・層移動に
追随しなかった形が 8 件(pr-233#10・pr-244#18・pr-258#11・pr-261#17・pr-267#10・pr-271#11・
pr-277#19・pr-298#13)、列挙・件数が同じ差分内の増減に追随しなかった形が 4 件(pr-258#16・
pr-277#18・pr-286#21・pr-289#14)、Why / Why not の前提構造が同じ差分内の別の変更で崩れた
形が 9 件(pr-258#15・pr-269#16・pr-274#17・pr-277#17・pr-282#15・pr-286#20・pr-298#14・
pr-301#14・pr-305#10)、doc の物理的な付着先・行幅が機械的にずれた形が 3 件(pr-227#15・
pr-291#5・pr-267#17)で、原因の性質が異なる 4 種が 1 タグに畳まれていた。残り 1 件
(pr-267#11。移動後に別 feature を指すことになるコメントの扱いを決めていなかった)は単発の
指摘のため、新語彙には割らず旧タグに残す(1 件しか出ていない分類は何もしない)。

層は 4 語彙とも観点のまま据え置いた。`.claude/agents/implementation-reviewer.md`「同じ事実を
説明する記述が差分の複数箇所にあるとき」の表に、確かめ方ごとの新タグを振る `分類` 列を足し、
4 件側(列挙・件数の追随漏れ)を新しい行として追加した。**フック化は次点候補**:
`comment-block-placement`(隣接する `/** */` ブロックの二重付着)は AST で機械判定できる
見込みが高いが、既存の `.claude/hooks/lib/missing-doc-comments.py` はコンパニオンオブジェクト
のメソッドのような入れ子の宣言を対象外にしており(`rules/coding.md` の対象と食い違う偽陽性を
避けるための既存の絞り込み)、対象を広げる改修と検証が要るため、この回では見送った。
`comment-block-placement` が今後 2 回再発したら、2a に戻ってフック化を検討する。
`comment-referent-drift`(改名・移動前の名前が残る形)は `grep` による絞り込みは機械的にできる
ものの、「絞り込んだ候補が本当に食い違いか」の最終判断は読解が要るため、一次フィルタとしての
