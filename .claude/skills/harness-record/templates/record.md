...
| `plan-open-decision-declaration` | `plan-review.md`「未決の判断が残っていないか」(計画の「未決の判断」欄を「無し」で締めたのに、実装中に手が止まる種類の判断が実際には残っていた形。欄そのものは確認されているが、「無し」という記載を疑わずに通している) |
| `plan-split-followup-omission` | AGENTS.md「タスクの分割」(却下・据え置き・分割すると決めた作業について、後続 Issue の起票・リンク・理由の記録のいずれかを欠いたまま計画・PR が進む形。判断そのものをどこにも書き残さない場合を含む) |
| `plan-derived-scope-omission` | implementation-flow フェーズ 3〜4(この変更が新しく開く経路・生む非対称・生みうる不正な状態を、計画のスコープ・完了条件が数え切れていない形。既存の不変条件を崩す `plan-implicit-condition` とは異なり、変更によって新しく生まれる分岐・経路を指す) |
...
スキル)も不要**(既存の `plan-review.md` の 1 項目を具体化・追加する話で、新しい段取りが
増えるわけではない)。過去の記録の `分類: plan` は書き換えない。

`plan` は本ルーティン実行時点で `plan-open-decision-declaration` / `plan-split-followup-omission` /
`plan-derived-scope-omission` へ分割した(skill 層まで介入済み(pr-317)なのに再発 50 件で飽和 —
「2c. 介入後 5 回以上」の閾値を 10 倍超えて突出)。50 件のうち、計画の「未決の判断」欄を「無し」で
締めたのに実装中に手が止まる判断が実際には残っていた形が 10 件(pr-332#9、pr-435#11、pr-442#9、
pr-459#15、pr-476#7、pr-495#8、pr-500#2、pr-512#5、pr-525#9、pr-539#5。すり抜け 0、全件
`plan-reviewer` が捕捉していたが計画作成側が繰り返した)、却下・据え置き・分割の判断を Issue や
計画へ記録し損ねる形が 7 件(pr-339#8、pr-442#28、pr-507#4、pr-516#12、pr-525#11、pr-533#12、
pr-539#23。うち pr-442#28 はすり抜け=人)、変更が新しく開く経路・生む非対称を計画のスコープが
数えていない形が 5 件(pr-400#4、pr-407#28、pr-438#3、pr-459#12、pr-526#16。うち pr-438#3 は
すり抜け=人)で、局面もすり抜けの性質も異なる 3 種が 1 タグに畳まれていた。残り 28 件は表示確認・
テスト配置の抜け、却下理由と採用節の矛盾、仕様未決定のまま実装、`harness-growth` 自身のメタ的な
指摘などが散在するが、いずれも 2〜4 件程度で新規タグに足る塊にならず、新語彙には割らず旧タグに
残す。

層は 3 つとも観点とした(新設のため「据え置き」ではなく初期配置)。`.claude/agents/plan-reviewer.md`
「未決の判断が残っていないか」を `plan-open-decision-declaration` として、「無し」という記載を
疑わずに通さないことを明文化した。`plan-split-followup-omission` は却下案の観点の並びへ、
`plan-derived-scope-omission` は `plan-implicit-condition` の隣へ、それぞれ新しい観点として
追加した。**フック化は不成立**(「未決が本当に無いか」「後続 Issue の理由が妥当か」「新しく
開く経路を数え切れているか」は、いずれも自然言語の計画書を読んで判断する必要があり、文字列一致
では偽陽性しか出ない。ただし `plan-split-followup-omission` のうち「Issue 番号の参照が本文に
あるか」自体は正規表現で機械判定できるので、今後 2 回再発したら 2a に戻ってその部分だけの
フック化を検討する)。**skill 化(新しいスキル)も不要**(既存の `plan-review.md` の観点を
追加する話で、新しい段取りが増えるわけではない)。過去の記録の `分類: plan` は書き換えない。
