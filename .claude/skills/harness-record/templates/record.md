...
| `plan-scope-premise-verification` | 同上(フェーズ 3 手順 3)(想定した接続先・前提・検証手段(検証コマンド自体が意図した範囲を覆っているかを含む)が実例で本当に成立する/機能するかを、実際に動かして確かめていない形) |
...
| `plan-rejection-reasoning` | `plan-review.md`「却下案の理由が理由になっているか」(却下案・据え置き判断の理由が、実際のコード・仕様・時系列に照らして成立しているかを検算していない形。理由が事実と違う/実際に起こりやすい向きと逆/一時的な事実に依っている/対象の一部しか覆っていない、を含む) |
| `plan-rejection-coverage` | `plan-review.md`「却下案の一覧が、実在する近い代替案を網羅しているか」(却下案の一覧に、実在するいちばん近い代替案が挙がっておらず、却下の検討そのものが閉じないまま計画が進む形) |
...
**後続の棚卸しルーティンが `harness/records/pr-352.md` へ 3 行を追記し、この積み残しを
解消する。**
