...
# 追加行の行番号は .github/scripts/lib/added-lines.sh が数える(移動は rename として
# 追跡するので、動かしただけのファイルは追加行を持たない)。
# 既存行の抑制で落とさないよう、**追加された行に載っているものだけ**を違反とする。
# ただしファイルを分けると、切り出した側は rename として組めず全行が「追加」になる。
# base に同じ綴りの行があるものはそこで除く。
set -euo pipefail
...
sources=('*.ts' '*.tsx' '*.js' '*.jsx')

# base に既にあった抑制コメントの綴り。ファイル分割で切り出した側を「追加」と読まないために
# 使う(分割は元のファイルが残るので rename として組めず、切り出した側は全行が追加行になる)。
# 抑制コメントは理由を必ず後ろに書くので、綴りが一字一句同じなら同じ抑制が移ったもの。
existing_suppressions="$(
  git grep -h -E 'biome-ignore|eslint-disable' "$base" -- "${sources[@]}" 2>/dev/null |
