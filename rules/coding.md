...
2. **その形にした理由** — なぜそう書いたか(この規約と記録では Why と呼ぶ)
3. **採らなかった理由** — なぜ他の選択肢を採らなかったか(同じく Why not)

**呼び名をコメントに書かない。** `Why:` / `Why not:` をラベルとして頭に置かず、理由の文だけを書く(「〜にはしない。〜だから」と書けば、採らなかった理由であることは文から読める)。

### doc に書く項目

**概要は 1〜2 文で書く。** 3 文以上になったら、下の「消すのは」に当たるものが混ざっているか、理由を概要へ畳んでいるか(理由は段落を分ける)、その関数が2つの役割を持っている。**挙動が及ぶ対象の列挙は概要ではなく仕様**なので、例示と混ぜて削らない(落とすと入力・出力・画面のどれかが変わるなら列挙、変わらないなら例示)。概要と列挙に加えて、**持っている項目だけ**を次の形で書く。
...
**まず、素直に書いてそうなった実装に理由は要らない。** 単純な処理・自明な分岐に「なぜこう書いたか」を足さない。書くのは、読み手が「こっちのほうが素直では」と考えてやり直しかねない箇所だけ。そこで採った理由だけを書くと「他にどんな道があったか」が残らず、次に読む人が同じ検討をやり直すので、採らなかった理由も一緒に書いて検討を一度で終わらせる。

- **採らなかった理由を書く相手は、実際に検討して捨てた案だけ。** 思いつく限りの代案を並べない(それは Issue の担当)
- **理由の根拠に、いま直されつつある一時的な事実を置かない。** 直った瞬間に理由が嘘になる
- **理由は1つにつき1〜2行**に収める。3行以上要るなら、それは Issue に書くべき経緯が紛れ込んでいる
- 逆に、**消してはいけない理由**もある: 型やテストで表現できず、外すと壊れるのに壊れたことに気づけないもの

消すのは **What の言い換え** / **コード内の値の再掲** / **1 つ挙げれば足りる例示** / **経緯・実行履歴** / **Issue の見出し番号だけを指すもの** / **一般論**。判断軸は「これを消したら、次に読む人が同じ間違いをするか」。しないなら消す。
