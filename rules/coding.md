...
**概要は 1〜2 文で書く。** 3 文以上になったら、使う側が知らなくてよいこと(実装を読めば分かる手順の言い換え・例示・経緯)が混ざっているか、その関数が2つの役割を持っている。ただし**挙動が及ぶ対象の列挙は概要ではなく仕様**なので削らない(「## ルール」の「対象を列挙する」)。概要に加えて、**持っている項目だけ**を次の形で書く。
