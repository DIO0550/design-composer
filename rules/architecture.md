...
src/
  ...
  services/    # ドメインサービス層: 原則使わない(後述「services は原則作らない」)
...
- **`src/services/` は原則使わない**(後述)。「複数ドメインに跨る」と感じたロジックも、まず帰属先のドメインオブジェクトを探す
...
**`services/` に新しいモジュールを追加しない。** ロジックには必ず帰属先の型がある、というのがこのリポジトリの立場であり、`services/` はその帰属先を見つけられなかったコードの吹きだまりになりやすい。

「これは複数ドメインに跨るからサービスだ」と感じたときは、**サービスを作る前に次の順で置き場所を探す**。ほとんどの場合ここで解決する。
...
1〜6 のどれにも当てはまらないと判断した場合のみ `services/` の候補になるが、**新規追加は実装前に確認する**(勝手に作らない)。「どちらの型にも帰属しない調停そのものが目的」であることを、選択肢と根拠つきで説明できることが条件。

### 既存 / 例外的な services の制約
