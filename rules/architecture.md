...
- **実装が1ファイルで完結する場合は `index.ts` に直接書き、実装ファイルを分けない**。複数の実装ファイルへの分割が必要になった場合のみ実装ファイルを分け、`index.ts` から re-export する。その場合も**1フォルダに実装ファイルは1つ**とし、2つ以上必要になったらサブフォルダに分割する(サブフォルダも同じ形を保つ)
