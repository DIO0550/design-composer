...
この 3 つの語彙は、`なし` として溜まっていた 14 件を `harness-growth`（当時の「逃し弁」）で
...
確かめ方は 3 つ。`harness-growth` の Step 3「介入先を決める」に手順として入っている。
...
この判断は Issue #620 で更新された（介入先は `hook` と `case-law` の 2 択になり、`skill` /
`観点` / `rules` は層から外れた）。下の実測は 4 層だった頃のもので、偏りの根拠としては生きている。

`rules`（層 4）へ落とす前に `hook`（層 1）を linter 設定・自前スクリプトの両方で確かめる、が
実際には飛ばされがちだった。介入 38 件中 28 件が `観点`（層 3）へ降りている（pr-272 時点）。
`harness-growth` の Step 3 に「フック化は不成立と書くときは両方確かめたうえで書く」を
足したのはこの偏りが根拠。

## セッションの URL を Issue に残す — 機械で強制できない理由

この判断は Issue #620 で更新された（層の語彙が `hook` / `case-law` の 2 つになったため、
下の「層 3」はフック = `hook` を指す）。
...
この 5 つの語彙は、`なし` として溜まっていた 23 件を `harness-growth`（当時の「逃し弁」）で
