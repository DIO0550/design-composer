...
## 規約の適用範囲 — base との差分を「追加」と読み違える

`.github/scripts/check-added-*.sh` は「このブランチで追加した行」だけを見る検査で、
**行番号だけで追加を判定すると、ファイル分割による移動を新規追加と誤検知する**。実際に
どちらの検査がこの余地を塞いでいて、どちらに残っているかは `.claude/hooks/README.md`
「例外(エスケープハッチ)」の `check-added-*` の項が持つ。
